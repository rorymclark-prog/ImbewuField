...
The start, middle and held end frames were inspected, including a 390px rendition. In the local 390px Student player on slide 43, the video loaded with `readyState=4`, no media error and no loop; it reached `ended=true` and remained on its final frame. The player offered the 48 KB clip on demand. See `plant-guilds-thinning.md` for reproducible build, exact bytes and source alignment.
