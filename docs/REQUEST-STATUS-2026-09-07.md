...
| --- | --- | --- | --- |
| R01 | Make site reports visually rich with charts, graphics, photographs and maps. | Existing; further design review open | `ReportView`, `ReportVisualOverview`, `report-visuals` and `report-visual-pdf` provide a photo/plan cover, headline figures, area, rainfall, storage, sowing calendar and priced BOQ charts. The user's request for a substantially more inspiring result still calls for visual review; component presence alone does not settle that judgement. |
