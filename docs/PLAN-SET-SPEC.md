...
## The canonical 9 sheets (Rory's spec, updated 2026-08-02)
...
| 05 | Earthworks Plan | ✅ Earthworks Blueprint (`buildBlueprintEarthworksMap`) |
| 06 | Planting & Agroforestry Plan | ✅ Planting Blueprint (`buildBlueprintPlantingMap`) |
| 07 | Small Livestock & Infrastructure Plan | ✅ Structures Blueprint (`buildBlueprintStructuresMap`) |
| 08 | Final Integrated Permaculture Masterplan | "Whole design" (still the plain composite) |
| 09 | Implementation Map & Phasing | ✅ `buildImplementationMap` |
...
nine above are sufficient (Rory's call).
...
renumber the Print composer (DesignPrint) and the Glossy design-map tabs to 01–09 above; (c) keep
"Whole design" (08 Masterplan) separate from Implementation (09). Sheets 03/04/05/06/07/09 already
exist as Blueprints or deterministic maps; 01 needs terrace levels + existing features; 02 is the new
build.
...
## Sheet 09 — Implementation & Phasing (the differentiator)
...
2. ~~**Planting + Livestock Blueprints** (sheets 06/07)~~ — **built.** Elements draw at TRUE
   footprint (wM/hM are real metres), colour-coded per SPECIES with a present-only legend.
3. **Implementation/phasing engine** (sheet 09) — not built; biggest product win.
...
styling.** Chase sheets 04/05/07/09 deterministically before chasing prettier pixels.
