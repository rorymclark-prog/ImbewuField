...
function ProductionAreaContent({ publishedOnly }: { publishedOnly: boolean }) {
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    {(manage || sample) && !publishedOnly && <form className={styles.card} style={{ marginTop: 24 }} onSubmit={e => { e.preventDefault(); void save(); }}><fieldset disabled={sample} style={{ minWidth: 0 }}><h2>{editing ? 'Update this garden' : 'Add a garden observation'}</h2>
