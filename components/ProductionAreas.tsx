...
function ProductionAreaContent({ publishedOnly }: { publishedOnly: boolean }) {
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    {sample ? <p className={styles.card}>Sample only · the starting areas come from the illustrative Ubhejane design, not a verified field observation. Edit, save and change sharing here; no real project is changed.</p> : <label>Organisation<select disabled={saving} value={org} onChange={e => setOrg(e.target.value)}>{orgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}</select></label>}
