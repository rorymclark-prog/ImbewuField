...
import { sampleRead, sampleWrite, freshSampleAreas, upsertSampleArea } from '@/lib/sample-operations';
...
function ProductionAreaContent({ publishedOnly }: { publishedOnly: boolean }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (isSampleMode()) { sampleWrite('areas', upsertSampleArea(sampleRead('areas', freshSampleAreas), { ...site, updatedAt: new Date().toISOString(), updatedBy: 'sample-organisation' } as ProductionSite, new Date().toISOString().slice(0, 10))); }
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    {sample ? <p className={styles.card}>Sample only · the starting areas come from the illustrative Ubhejane design, not a verified field observation. {publishedOnly ? 'Only areas shared by the sample organisation appear here.' : 'Edit, save and change sharing here; no real project is changed.'}</p> : <label>Organisation<select disabled={saving} value={org} onChange={e => setOrg(e.target.value)}>{orgs.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}</select></label>}
