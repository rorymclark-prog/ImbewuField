...
export default function OrganisationControlCentre() {
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    {tab === 'preview' && <><p className={styles.notice}>These previews use fictional demonstration records. They show each role’s workspace without signing in as another person or changing anyone’s permissions. They do not impersonate a particular user’s live data or custom access.</p><div className={styles.grid}>{([['farmer', 'Farmer'], ['mentor', 'Mentor / extension officer'], ['student', 'Student'], ['ngo', 'Organisation'], ['funder', 'Funder']] as const).map(([role, label]) => <button key={role} className={styles.card} onClick={() => { if (startRolePreview(role)) router.push(`/${role}`); else setError('The practice workspace could not open. Please try again.'); }}><h2>{label}</h2><p>Open workspace →</p></button>)}</div>{error && <p role="alert">{error}</p>}</>}
  </div></section>;
}
