...
export default function OrganisationControlCentre() {
  ...
  const [tab, setTab] = useState<'people' | 'teams' | 'preview'>('people');
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    <div className={styles.row}>{([['people', 'People & permissions'], ['teams', 'Mentor teams'], ['preview', 'View as a role']] as const).map(([key, label]) => <button key={key} aria-pressed={tab === key} onClick={() => setTab(key)}>{label}</button>)}</div>
    {tab === 'people' && <><p className={styles.card}>Set a member’s role, delegate assessment management and private analysis, choose who manages people, and enable or withdraw linked funder access. Production areas and assessment summaries also have individual publication controls in their sections.</p><MelDashboard accessOnly /></>}
