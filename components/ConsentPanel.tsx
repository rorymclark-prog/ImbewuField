    ...
    <div className="rounded-2xl px-4 py-4" style={{ background: 'var(--bg-1)', border: '1px solid var(--border)' }}>
      <div className="flex items-center gap-2">
        ...
        <div className="text-xs font-mono uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>
          ...
          <span>{CONSENT_PANEL_HEADING.labelZuDraft} <span className="font-sans text-[10px]" style={{ color: 'var(--text-muted)' }}>(isiZulu machine draft)</span></span>
        </div>
      </div>
      <p className="text-sm font-display mt-3 leading-snug" style={{ color: 'var(--text-secondary)' }}>
        ...
        <span className="block mt-1">Akukho lutho olwabiwayo lapha ngaphandle kokuthi uyivule. Ungashintsha umqondo wakho noma nini, futhi {whoZu} izoyeka ukukubona ngokushesha.</span>
      </p>

      {loading ? (
        <div className="flex items-center gap-2 py-4 text-sm font-display" style={{ color: 'var(--text-muted)' }}>
          <Loader2 size={14} className="animate-spin" /> <span><span className="block">Loading…</span><span className="block">Kusalayishwa… <span className="font-semibold">(isiZulu machine draft)</span></span></span>
        </div>
      ) : (
        <>
          <div className="mt-3">
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    <span className="block text-sm font-display mt-0.5" style={{ color: 'var(--text-primary)' }}>{labelZuDraft} <span className="text-[10px] font-sans" style={{ color: 'var(--text-muted)' }}>(isiZulu machine draft)</span></span>
                    <span id={`consent-detail-${id}`} className="block text-xs font-display mt-0.5" style={{ color: 'var(--text-muted)' }}>
                      <span className="block">{detail}</span>
                      <span className="block mt-1">{detailZuDraft} <span className="font-sans text-[10px]">(isiZulu machine draft)</span></span>
                    </span>
                  </span>
          ...
          {error && (
            <div className="mt-3 rounded-lg px-3 py-2 text-xs font-display" style={{ background: '#F6E7E1', color: '#8A3B1C' }}>
              ...
              <span className="block mt-1">Lokho akugcinwanga. Izilungiselelo zakho zokwabelana azishintshanga. <span className="font-sans">(isiZulu machine draft)</span></span>
            </div>
          )}
          ...
          <div className="flex flex-col items-start gap-2 mt-4 pt-3" style={{ borderTop: '1px solid var(--border)' }}>
            <span className="text-xs font-display" style={{ color: 'var(--text-muted)' }}>
              ...
              <span className="block mt-0.5">{on === 0 ? 'Awabelani ngalutho.' : `Wabelana ngezinto ezingu-${on} kwezingu-${CONSENT_SCOPES.length}.`} <span className="font-sans text-[10px]">(isiZulu machine draft)</span></span>
            </span>
            <button
              type="button"
              onClick={stopAll}
            ...
            >
              ...
              <span><span className="block">Stop sharing everything</span><span className="block">Yeka ukwabelana ngakho konke <span className="font-sans text-[10px]">(isiZulu machine draft)</span></span></span>
            </button>
          </div>
        </>
      )}
