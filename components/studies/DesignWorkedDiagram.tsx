...
function Dimension({ feature, horizontal, label, colour = '#715634', side = 'left' }: { feature: RectFeature; horizontal: boolean; label: string; colour?: string; side?: 'left' | 'right' }) {
  ...
  const dimX = side === 'right' ? r.x + r.width + 28 : r.x - 18;
  const textX = side === 'right' ? dimX + 15 : dimX - 7;
  return <g fill={colour} stroke={colour} strokeWidth="1.5" fontSize="12" fontFamily="Arial, sans-serif"><path d={`M ${dimX} ${r.y} V ${r.y + r.height} M ${dimX - 5} ${r.y} H ${dimX + 5} M ${dimX - 5} ${r.y + r.height} H ${dimX + 5}`} /><text x={textX} y={r.y + r.height / 2} textAnchor="middle" stroke="none" transform={`rotate(-90 ${textX} ${r.y + r.height / 2})`}>{label}</text></g>;
}
...
export default function DesignWorkedDiagram({ view = 'source', svgRef }: { view?: DesignWorkedDiagramView; svgRef?: RefObject<SVGSVGElement> }) {
  ...
  const viewRevision = isRevision ? DESIGN_WORKED.revision.to : DESIGN_WORKED.revision.from;
  ...
  return <figure className={styles.figure}>
    ...
    <div className={styles.viewport} tabIndex={0} role="region" aria-label="Scrollable fictional classroom yard-plan diagram">
      <svg ref={svgRef} className={`${styles.diagram}${enlarged ? ` ${styles.enlarged}` : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 802" width="720" height="802" role="img" aria-labelledby="worked-diagram-title worked-diagram-description" fontFamily="Arial, sans-serif">
        ...
        <rect width="720" height="802" fill="#f7f1e3" />
        ...
        <text x="32" y="90" fill="#8b4226" fontSize="11" fontWeight="700">FICTIONAL CLASSROOM MODEL · VIEW {viewRevision} · CURRENT PACK {DESIGN_WORKED.currentRevision} · NOT A SURVEY OR CONSTRUCTION DRAWING</text>
        ...
        <g aria-label="Teaching frame and existing evidence">
          ...
          <path d={`M ${x(water.x) - 8} ${y(water.y) - 5} L ${x(water.x) - 42} ${y(water.y) - 17}`} stroke="#31593a" strokeWidth="1.5" fill="none" />
          <text x={x(water.x) - 45} y={y(water.y) - 20} fill="#274d36" fontSize="11" textAnchor="end">Water point</text>
        ...
        {isA && <g aria-label="Concept A proposal"><rect {...compostRect} fill="#ead19c" stroke="#7a4d20" strokeWidth="3" strokeDasharray="7 5" /><rect {...compostRect} fill="url(#compost-hatch)" /><NumberKey id="6" cx={compostRect.x + compostRect.width / 2} cy={compostRect.y + compostRect.height / 2} /><Dimension feature={compost} horizontal label={`${compost.width} m`} colour="#7a4d20" /></g>}
        {isB && <g aria-label="Concept B alternative proposal"><rect {...bPlot} fill="#f8dcad" fillOpacity="0.8" stroke="#b85f1c" strokeWidth="3" strokeDasharray="9 6" /><NumberKey id="7" cx={bPlot.x + bPlot.width - 17} cy={bPlot.y + 18} /><Dimension feature={plotB} horizontal label={`${plotB.width} m`} colour="#a6501a" /><Dimension feature={plotB} horizontal={false} label={`${plotB.height} m`} colour="#a6501a" side="right" /></g>}
        ...
        {isRevision && <g aria-label="Deferred compost revision"><rect {...compostRect} fill="none" stroke="#8b2e1e" strokeWidth="3" strokeDasharray="5 4" /><path d={`M ${compostRect.x} ${compostRect.y} L ${compostRect.x + compostRect.width} ${compostRect.y + compostRect.height} M ${compostRect.x + compostRect.width} ${compostRect.y} L ${compostRect.x} ${compostRect.y + compostRect.height}`} stroke="#8b2e1e" strokeWidth="2" /><text x="244" y={plan.y + plan.height + 40} fill="#8b2e1e" fontSize="12" fontWeight="700">P-COMPOST deferred — R2</text><text x="244" y={plan.y + plan.height + 56} fill="#8b2e1e" fontSize="11">W-CARE: no agreed care role</text></g>}
        ...
        <g aria-label="Reported water observation note" fill="#31566c"><text x="56" y={plan.y + plan.height + 70} fontSize="11" fontWeight="700">W-OBS: reported eastern-yard water issue</text><text x="56" y={plan.y + plan.height + 85} fontSize="11">Water reportedly crosses the eastern yard in heavy rain.</text><text x="56" y={plan.y + plan.height + 100} fontSize="11">Extent and levels are unmeasured; this is not a flood boundary.</text></g>

        <g aria-label="Legend and provenance" transform="translate(32 590)">
          <rect x="0" y="0" width="656" height="190" rx="8" fill="#fffdf7" stroke="#d5c8af" />
