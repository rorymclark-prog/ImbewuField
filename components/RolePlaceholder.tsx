  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: 'var(--bg-0)' }}>
      <header className="flex-shrink-0 flex items-center px-3 md:px-5 gap-2 md:gap-4 overflow-x-auto"
        ...
        <span className="text-xs hidden sm:block font-display" style={{ color: 'var(--text-muted)' }}>{subtitle}</span>
      ...
      <div className="flex-1 overflow-y-auto flex items-center justify-center p-6">
        <div className="w-full max-w-lg rounded-2xl p-7 text-center" style={{ background: 'var(--bg-1)', border: '1px solid var(--border-bright)' }}>
          ...
          <p className="font-display text-sm leading-relaxed mb-6" style={{ color: 'var(--text-muted)' }}>{blurb}</p>
          <div className="grid grid-cols-1 gap-2 text-left">
            {features.map((f) => (
              <div key={f.label} className="flex items-center gap-3 px-3 py-2.5 rounded-xl" style={{ background: 'rgba(22,37,20,0.5)', border: '1px solid var(--border)' }}>
