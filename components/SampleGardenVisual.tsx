...
export default function SampleGardenVisual({ name = 'Demonstration garden', initial = 'design', kind = 'Community garden', variant = name }: { name?: string; initial?: 'design' | 'aerial'; kind?: string; variant?: string }) {
  ...
  return <section style={{ background:'#eef2e7', padding:12, height:'100%', overflow:'auto' }} aria-label={`Garden visual: ${name}`}>
    ...
    <img data-photo-preview src={view==='design'?sampleGardenImage(kind,variant):(photo ?? '/demo/aerial.webp')} alt={view==='design'?`Schematic ${kind.toLowerCase()} layout for ${name}. Not a measured site plan.`:`AI-generated site reference for ${name}. Not field evidence.`} style={{ display:'block', width:'100%', maxWidth:780, margin:'auto', borderRadius:16 }} />
    <p style={{ fontSize:14, color:'#3f533e', marginTop:10 }}>{view==='design'?`${kind} · ${name}. Tap the layout to enlarge. Schematic layout; not to scale.`:`AI-generated reference for ${name}. Tap to enlarge. Illustrative setting, not satellite imagery or a measured site plan.`}</p>
  </section>;
}
