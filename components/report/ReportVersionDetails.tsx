...
export default function ReportVersionDetails({ settings, language, savedAt, reference, sample = false }: {
  ...
  return <details className={styles.versionDetails}>
    <summary>
      <strong>{savedAt ? `Saved report · ${date(savedAt)}` : sample ? "Prepared report" : 'New report · not saved yet'}</strong>
    ...
    <dl>
      ...
      <div><dt>AI provider / model</dt><dd>{sample && !settings ? 'Prepared report; no AI charge' : [settings?.provider, settings?.model].filter(Boolean).join(' / ') || 'Not recorded'}</dd></div>
    </dl>
