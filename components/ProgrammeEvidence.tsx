    ...
    {data&&<>
      ...
      {!data.brandingOnly&&tab==='training'&&<>
        ...
        {session&&<form className={styles.card} style={{marginTop:20}} onSubmit={e=>{e.preventDefault();void save('session');}}>
          <h2>{data.canRecord?'Session record & attendance':'Published session report'}</h2><div className={trainingStyles.summary}>{[['Present',session.presentCount],...(!funder?[['Signed',trainingEvidenceSummary(session).signed],['Certificates',trainingEvidenceSummary(session).certificates]]:[]),['Photographs',session.photos.length]].map(([label,value])=><div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
          <fieldset disabled={!data.canRecord||busy||photosBusy} style={{minWidth:0}}>
            ...
            {!funder&&<><h3>Attendance register</h3><p>Keep only the people expected at this session on the register, then tick Present for those who attended. Reuse guest codes across sessions so each person counts once.</p><div className={trainingStyles.register}>{session.attendance.map(p=><article key={p.id} className={trainingStyles.person}><div><label className={styles.option}><input type="checkbox" checked={p.present} onChange={e=>{const present=e.target.checked;setSession(s=>s?{...s,attendance:s.attendance.map(a=>a.id===p.id?{...a,present,...(!present?{signature:undefined,certificate:undefined}:{})}:a),feedback:present?s.feedback:s.feedback?.filter(f=>f.participantId!==p.id)}:s);}}/><strong>{p.name}</strong></label><span>{p.present?'Present':'Absent'} · {p.id.startsWith('guest-')?'Guest':'Enrolled'}</span><label>ID / participant reference (optional)<input maxLength={80} value={p.reference??''} onChange={e=>setS('attendance',session.attendance.map(a=>a.id===p.id?{...a,reference:e.target.value,signature:undefined}:a))}/></label></div><div>{p.present?<TrainingSignature name={p.name} value={p.signature} onChange={signature=>setS('attendance',session.attendance.map(a=>a.id===p.id?{...a,signature}:a))}/>:<p>Mark present to sign attendance.</p>}</div><div><label>Certificate reference<input maxLength={80} disabled={!p.present} value={p.certificate??''} placeholder="When issued" onChange={e=>setS('attendance',session.attendance.map(a=>a.id===p.id?{...a,certificate:e.target.value}:a))}/></label><button type="button" aria-label={`Remove ${p.name} from register`} onClick={()=>setSession(s=>s?{...s,attendance:s.attendance.filter(a=>a.id!==p.id),feedback:s.feedback?.filter(f=>f.participantId!==p.id)}:s)}>Remove from register</button></div></article>)}</div><label>Add an enrolled participant<select value="" onChange={e=>{const p=data.people.find(p=>p.id===e.target.value);if(p)setS('attendance',[...session.attendance,{...p,present:false}]);}}><option value="">Choose a participant</option>{data.people.filter(p=>!session.attendance.some(a=>a.id===p.id)).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
