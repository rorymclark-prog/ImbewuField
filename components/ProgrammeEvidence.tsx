...
const emptySession=():TrainingRecord=>({id:'',project:'',title:'',date:today(),venue:'',latitude:null,longitude:null,facilitator:'',ownerId:'',attendance:[],presentCount:0,registeredCount:0,report:'',nextSteps:'',assessmentId:'',published:false,photos:[],photoCount:0,updatedAt:''});
const emptyTarget=():ProgrammeMilestone=>({id:'',project:'',title:'',unit:'sessions',baseline:null,target:1,due:today(),owner:'',method:'',published:false,observations:[],updatedAt:''});
export default function ProgrammeEvidence({ funder=false, mentor=false, initialTab='progress' }: { funder?:boolean; mentor?:boolean; initialTab?:Tab }) {
  ...
  async function save(action:'session'|'milestone'|'branding') {
    ...
    try{
      ...
      const s=session ? {...session,id:session.id||crypto.randomUUID(),ownerId:session.ownerId||'sample-mentor',presentCount:session.attendance.filter(a=>a.present).length,registeredCount:session.attendance.length,photoCount:session.photos.length} : null;
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    <div className={styles.hero}><h1>{funder?'Project progress & evidence':'Training, evidence & progress'}</h1><p>Attendance, session records and dated deliverables in one place. Print a progress report whenever you need it.</p></div>
    {orgs.length>1&&<label>Organisation<select value={org} onChange={e=>setOrg(e.target.value)}>{orgs.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select></label>}
    ...
    {data&&<>
      ...
      <div className={styles.row}>{((data.brandingOnly?['branding']:['progress','training',...(!funder?['targets']:[]),...(data.canBrand?['branding']:[])]) as Tab[]).map(t=><button key={t} aria-pressed={tab===t} onClick={()=>setTab(t)}>{{progress:'Progress report',training:'Training register',targets:'Milestones',branding:'Names & logos'}[t]}</button>)}</div>
      <label>Project<select value={project} onChange={e=>setProject(e.target.value)}><option value="">All visible projects</option>{projects.map(p=><option key={p}>{p}</option>)}</select></label>
      {!data.brandingOnly&&tab==='progress'&&<>
        <div className={styles.card}><label>Progress as of<input type="date" value={asOf} max={today()} onChange={e=>{if(e.target.value)setAsOf(e.target.value);}} /></label><label>Timeline · recorded dates<input aria-label="Progress timeline" type="range" min={0} max={Math.max(0,dates.length-1)} value={Math.max(0,dates.findLastIndex(d=>d<=asOf))} onChange={e=>setAsOf(dates[+e.target.value])} /></label><p>Shows observations dated on or before {asOf}, using the latest corrected records. This is not a reconstruction of what had been entered at that time.</p></div>
      ...
      {!data.brandingOnly&&tab==='training'&&<>
        {data.canRecord&&<button className={styles.primary} onClick={()=>{setSession({...emptySession(),project,attendance:data.people.map(p=>({...p,present:false})),facilitator:profile?.full_name??(data.sample?'Sample mentor':'')});setReviewed(false);}}>Record a training session</button>}
        {photosBusy&&<p>Loading session photos…</p>}
        {sessions.map(s=><article key={s.id} className={styles.card} style={{marginTop:16}}><h2>{s.title}</h2><p>{s.date} · {s.project} · {s.venue}</p><p>{s.presentCount} present / {s.registeredCount} registered · {s.photoCount} venue photos · {s.published?'Shared with funders':'Internal'}</p><p>{s.report}</p><button disabled={photosBusy} onClick={()=>void editSession(s)}>{data.canRecord?'Open register / edit':'Open session report'}</button></article>)}
        {session&&<form className={styles.card} style={{marginTop:20}} onSubmit={e=>{e.preventDefault();void save('session');}}>
          ...
          <fieldset disabled={!data.canRecord||busy||photosBusy} style={{minWidth:0}}>
            ...
            {!funder&&<><h3>Attendance register</h3><p>Tick Present for attendees. Reuse each guest’s stable code across sessions so they count once.</p>{[...data.people,...session.attendance.filter(a=>!data.people.some(p=>p.id===a.id))].map(p=>{const a=session.attendance.find(a=>a.id===p.id);return <label className={styles.option} key={p.id}><input type="checkbox" checked={a?.present??false} onChange={e=>setS('attendance',[...session.attendance.filter(a=>a.id!==p.id),{id:p.id,name:p.name,present:e.target.checked}])}/>{p.name} · Present</label>;})}
            ...
            {data.canRecord&&session.photos.length<2&&<label>Add venue photo (up to two)<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async e=>{const file=e.target.files?.[0];if(!file)return;setPhotosBusy(true);try{const image=await resizeLogoForStorage(file,640);setSession(s=>s?{...s,photos:[...s.photos,{image,caption:''}]}:s);}catch(e){setError((e as Error).message);}finally{setPhotosBusy(false);}}}/></label>}
          ...
          <button type="button" onClick={()=>setSession(null)}>Close record</button>
