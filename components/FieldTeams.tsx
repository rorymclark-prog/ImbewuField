...
import type { FieldVisit, VisitPhoto } from '@/lib/field-teams';
...
export default function FieldTeams({ organisation = false }: { organisation?: boolean }) {
  ...
  const [visit, setVisit] = useState<Omit<FieldVisit,'mentorId'>>({ id: '', farmerId: '', date: new Date().toISOString().slice(0, 10), notes: '' }), [notice, setNotice] = useState('');
  const [org, setOrg] = useState('');
  const [visitPhotos,setVisitPhotos]=useState<Record<string,VisitPhoto[]>>({});
  async function loadPhotos(id:string){try{const res=await fetch(`/api/field-teams?visit=${encodeURIComponent(id)}${org?`&org=${encodeURIComponent(org)}`:''}`,{headers:await paidApiHeaders()});const result=await res.json();if(!res.ok)throw Error(result.error);setVisitPhotos(previous=>({...previous,[id]:result.photos}));}catch(e){setError((e as Error).message);}}
  const request = async (body?: unknown) => {
    if (isSampleMode()) throw Error("requests must stay in the demo.");
  ...
  useEffect(() => { let cancelled = false; setData(null); setVisitPhotos({}); setError(''); if (isSampleMode()) { setData(projectFieldWorkspace(completeSampleFieldWorkspace(sampleRead('field-teams', freshFieldWorkspace)), organisation ? 'sample-organisation' : 'sample-mentor', organisation)); return; } if (!user) return; void request().then(d => { if (!cancelled) setData(d); }).catch(e => { if (!cancelled) setError(e.message); }); return () => { cancelled = true; }; }, [user, org, organisation]); // eslint-disable-line react-hooks/exhaustive-deps
  async function save(team: boolean) {
    ...
    try {
      ...
      } else { if (data?.sample) throw Error('This practice workspace has ended. Reopen the workspace.'); await request(team ? { action: 'team', team: draft } : { action: 'visit', ...visit }); }
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    {data && <>
      ...
      {data.teams.map(t => <article key={t.mentorId} className={styles.card} style={{ marginBottom: 16 }}><h2>{t.location}</h2><p style={{ display: 'flex', alignItems: 'center', gap: 12 }}>{data.sample && <img data-photo-preview src={samplePortrait(t.mentorId)} alt="Mentor portrait illustration" width={72} height={72} style={{ borderRadius: '50%', objectFit: 'cover' }} />}<span>Mentor: {name(t.mentorId)} · updated {t.updatedAt.slice(0, 10)}</span></p><h3>Organisation guidance</h3><p style={{ whiteSpace: 'pre-wrap' }}>{t.guidance || 'No guidance posted yet.'}</p><h3>Assigned farmers and learners</h3>{t.farmerIds.map(id => <p key={id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>{data.sample && <img data-photo-preview src={samplePortrait(id)} alt="Profile portrait illustration" width={52} height={52} style={{ borderRadius: '50%' }} />}<span>{name(id)}{data.people.find(p => p.id === id)?.gardenName && <small style={{ display: 'block', fontSize: 13, color: '#526454' }}>{data.people.find(p => p.id === id)?.gardenName} · {Math.round(data.people.find(p => p.id === id)?.gardenAreaM2 ?? 0).toLocaleString()} m²</small>}</span></p>)}{data.canManage && <button onClick={() => setDraft(t)}>Edit team & guidance</button>}</article>)}
      ...
      {!organisation && assigned.length > 0 && <form className={styles.card} onSubmit={e => { e.preventDefault(); void save(false); }}><h2>Record a field visit</h2><label>Farmer<select required value={visit.farmerId} onChange={e => setVisit({ ...visit, id: visit.id || crypto.randomUUID(), farmerId: e.target.value })}><option value="">Choose a farmer</option>{assigned.map(id => <option key={id} value={id}>{name(id)}</option>)}</select></label><label>Visit date<input required type="date" max={new Date().toISOString().slice(0, 10)} value={visit.date} onChange={e => setVisit({ ...visit, date: e.target.value })} /></label><VisitCapture value={visit} disabled={busy} onBusyChange={setBusy} onChange={value=>setVisit({...visit,...value})}/><button className={styles.primary} disabled={busy}>Save visit</button></form>}
      <section className={styles.card}><h2>Recorded visits</h2>{data.visits.map(v=><article key={v.id} style={{marginTop:16}}><h3>{v.date} · {name(v.farmerId)}</h3><p style={{whiteSpace:"pre-wrap"}}>{v.notes}</p>{v.originalNotes&&<details><summary>Original notes before AI cleanup</summary><p style={{whiteSpace:"pre-wrap"}}>{v.originalNotes}</p></details>}{(v.photoCount??0)>0&&!(v.photos??visitPhotos[v.id])&&<button onClick={()=>void loadPhotos(v.id)}>View visit photos ({v.photoCount})</button>}<div style={{display:'flex',gap:12,flexWrap:'wrap'}}>{(v.photos??visitPhotos[v.id]??[]).map((p,i)=><figure key={i}><img data-photo-preview src={p.image} alt={p.caption||'Visit photograph'} style={{width:220,maxWidth:'100%',borderRadius:12}}/><figcaption>{p.caption}</figcaption></figure>)}</div></article>)}{!data.visits.some(v=>(v.photoCount??v.photos?.length??0)>0)&&<p>No visit photos recorded yet.</p>}</section>
      <ReportComposer photos={data.visits.flatMap(v=>(v.photos??visitPhotos[v.id]??[]).map(p=>({image:p.image,caption:`${v.date} · ${name(v.farmerId)} · ${p.caption}`})))} photoHeading="Visit photographs opened for this report" title={organisation ? 'Field implementation report' : 'Mentor field report'} sample={data.sample} sections={[
