...
// this component the same way FIELD_FOCUS's own isiZulu strings are.
const FIELD_PROGRAMMES_ZU: Record<FieldProgramme, string> = {
  general: 'Ukusekelwa okuvamile kwengadi',
  'act-sef-food-security': 'ACT · SEF ukuvikeleka kokudla',
};

// Maps FIELD_FOCUS's closed set of keys (lib/field-teams.ts) onto the MENTOR_ENGLISH_PENDING keys
// that now carry this component's isiZulu drafts for them (lib/locales/zu.ts), replacing the local
// FIELD_FOCUS_ZU literal this file used to keep next to FIELD_PROGRAMMES_ZU above.
const FIELD_FOCUS_KEYS: Record<FieldFocus, string> = {
  garden: 'mentorFocusGarden',
  water: 'mentorFocusWater',
  soil: 'mentorFocusSoil',
  seedlings: 'mentorFocusSeedlings',
  crops: 'mentorFocusCrops',
  harvest: 'mentorFocusHarvest',
  learning: 'mentorFocusLearning',
  records: 'mentorFocusRecords',
};
...
export default function FieldTeams({ organisation = false, initialFarmerId, onStartTraining }: { organisation?: boolean; initialFarmerId?: string; onStartTraining?:()=>void }) {
  ...
  // Preserves this component's already-existing isiZulu drafts (now real lib/locales/zu.ts
  // overrides, see lib/i18n-pending.ts's MENTOR_ENGLISH_PENDING comment) while routing every
  // other string through t(). English is shown alongside the draft the same way every other
  // unreviewed isiZulu draft in this app is (SurveyZuluDraftPair).
  const paired = (key: string) => lang === 'zu'
    ? <SurveyZuluDraftPair english={translate('en', key)}>{t(key)}</SurveyZuluDraftPair>
    : t(key);
  ...
  const programmeKeys=[...new Set(data?.teams.filter(t=>!farmerFilter||t.farmerIds.includes(farmerFilter)).map(t=>t.programme??'general')??[])];
  // Kept English — this feeds the exported field report (see `sections` below), which stays
  // English throughout regardless of `lang`, same as the rest of that report's own text.
  const programmeLabel=programmeKeys.length?programmeKeys.map(p=>FIELD_PROGRAMMES[p]).join(' / '):'Garden mentoring';
  // The on-screen counterpart, isiZulu-aware like the hero text it sits beside.
  const programmeLabelUi=programmeKeys.length?programmeKeys.map(p=>ui(FIELD_PROGRAMMES[p],FIELD_PROGRAMMES_ZU[p])).join(' / '):ui('Garden mentoring','Ukwelulekwa kwengadi');
  ...
  const actionLabel=(record:FieldVisit)=>record.actionCompletedOn?t('mentorActionCompleted'):record.followUpDate?record.followUpDate<today?t('mentorFollowUpOverdue'):record.followUpDate===today?t('mentorFollowUpToday'):t('mentorFollowUpScheduled'):t('mentorDateNeeded');
  ...
  return <section className={`${styles.root} ${workStyles.workspace}`}><div className={styles.wrap}>
    {lang === 'zu' && <p role="note" className={styles.notice}>{paired('mentorZuluDraftNotice')}</p>}
    ...
    <div className={workStyles.hero}>
      <div><p className={workStyles.eyebrow}>{organisation?paired('mentorProgrammeCoordination'):programmeLabelUi}</p><h1>{organisation?paired('mentorTeamsGuidanceHeading'):paired('mentorMyFieldwork')}</h1><p>{organisation?paired('mentorGiveEachMentorIntro'):paired('mentorSupportGardenIntro')}</p></div>
      {!organisation&&<div className={workStyles.heroActions}><button className={styles.primary} disabled={!canRecordVisits||busy||photosBusy||captureBusy} onClick={()=>newVisit()}>{paired('mentorRecordFieldVisit')}</button>{onStartTraining&&<button onClick={onStartTraining}>{paired('mentorTrainingAttendance')}</button>}</div>}
    </div>
    {profile?.role === 'admin' && !isSampleMode() && <label>{t('mentorOrganisationIdLabel')}<input value={org} onChange={e => setOrg(e.target.value)} placeholder={t('mentorOrganisationIdPlaceholder')} /></label>}
    {error && <p role="alert" className={styles.error}>{error}</p>}{notice && <p role="status" className={styles.notice}>{notice}</p>}
    {!data && !error && <p>{t('mentorLoadingFieldTeam')}</p>}
    {data && <>
      <nav className={workStyles.tabs} aria-label={t('mentorFieldworkViewsAria')}>{([['overview','mentorTabPriorities'],['people','mentorTabPeopleGardens'],['visits','mentorTabVisitRecords'],['report','mentorTabFieldReport']] as const).map(([key,labelKey])=><button key={key} aria-pressed={panel===key} onClick={()=>setPanel(key)}>{paired(labelKey)}</button>)}</nav>
      <details className={workStyles.scope}><summary>{t('mentorRecordScopePrefix')} · {scopeLabel}</summary><div className={styles.grid}>
        <label>{paired('mentorParticipantLabel')}<select value={farmerFilter} onChange={e=>{setFarmerFilter(e.target.value);setVisibleCount(10);}}><option value="">{t('mentorAllAssignedParticipants')}</option>{assigned.map(id=><option key={id} value={id}>{name(id)}</option>)}</select></label>
        <label>{t('mentorFromDateLabel')}<input type="date" max={to||today} value={from} onChange={e=>setFrom(e.target.value)}/></label>
        <label>{t('mentorToDateLabel')}<input type="date" min={from} max={today} value={to} onChange={e=>setTo(e.target.value)}/></label>
      </div><button onClick={()=>{setFrom('');setTo('');setFarmerFilter('');}}>{t('mentorClearFilters')}</button><p>{t('mentorFiltersExplain')}</p></details>
      {data.visitCursor&&<div className={styles.notice}><p>{t('mentorMoreHistoryNotice').replace('{count}',String(data.visits.length))}</p><button disabled={busy||captureBusy||photosBusy} onClick={()=>void loadMoreVisits()}>{busy?t('mentorLoadingEllipsis'):t('mentorLoadMoreVisitHistory')}</button></div>}
      {!scopeValid&&<p className={styles.error} role="alert">{t('mentorChooseEndDate')}</p>}
      {panel==='overview'&&summary&&scopeValid&&!data.visitCursor&&<>
        <div className={workStyles.metrics}>
          <div className={workStyles.metric}><span>{t('mentorMetricAssignedParticipants')}</span><strong>{summary.assigned.length}</strong><div className={workStyles.avatarStack}>{summary.assigned.slice(0,4).map(id=><span key={id}>{portrait(id,32)}</span>)}</div><small>{t('mentorMetricCurrentFieldTeam')}</small></div>
          <div className={workStyles.metric}><span>{t('mentorMetricParticipantsVisited')}</span><strong>{summary.visited} / {summary.assigned.length}</strong><div className={workStyles.coverage} role="img" aria-label={t('mentorCoverageAria').replace('{visited}',String(summary.visited)).replace('{unvisited}',String(summary.unvisited))}><i style={{width:`${summary.assigned.length ? summary.visited/summary.assigned.length*100 : 0}%`}}/></div><small>{t('mentorStillToVisit').replace('{count}',String(summary.unvisited))}</small></div>
          <div className={workStyles.metric}><span>{t('mentorMetricFollowUpsDue')}</span><strong>{summary.due.length}</strong><div className={workStyles.avatarStack}><CalendarClock size={28} className={workStyles.metricIcon} aria-hidden="true"/></div><small>{t('mentorOpenActionsDueToday')}</small></div>
          <div className={workStyles.metric}><span>{t('mentorMetricVisitsRecorded')}</span><strong>{summary.visits.length}</strong><div className={workStyles.avatarStack}><ClipboardCheck size={28} className={workStyles.metricIcon} aria-hidden="true"/></div><small>{t('mentorWithinSelectedPeriod')}</small></div>
        </div>
        <div className={workStyles.columns}>
          <section className={styles.card}><div className={styles.row}><h2>{t('mentorWhatNeedsAttention')}</h2><span className={styles.tag}>{t('mentorOpenActionsCount').replace('{count}',String(summary.open.length))}</span></div>
            {!summary.open.length&&<p>{t('mentorNoOpenActions')}</p>}
            {[...summary.due,...summary.open.filter(v=>!summary.due.includes(v))].slice(0,4).map(record=><article key={record.id} className={workStyles.action}><div className={workStyles.actionHeader}><div className={workStyles.personHeading}>{portrait(record.farmerId)}<div><strong>{name(record.farmerId)}</strong><p className={workStyles.personGarden}>{garden(record.farmerId)}</p></div></div><span className={record.followUpDate&&record.followUpDate<=today?workStyles.due:styles.tag}>{actionLabel(record)}</span></div><p>{record.agreedAction}</p><small>{record.responsiblePerson||t('mentorOwnerNeeded')} · {record.followUpDate||t('mentorAgreeFollowUpDate')}</small><button disabled={busy||captureBusy||photosBusy} onClick={()=>void openVisit(record)}>{t('mentorReviewAction')}</button></article>)}
            {summary.open.length>4&&<button onClick={()=>setPanel('visits')}>{t('mentorViewAllVisitActions')}</button>}
          </section>
          <aside className={styles.card}><FieldVisitGuide actProgramme={!!actProgramme}/></aside>
        </div>
        <section className={styles.card}><div className={styles.row}><Users size={28} aria-hidden="true"/><h2>{t('mentorPlanNextVisit')}</h2><span className={styles.tag}>{t('mentorParticipantsWithoutVisit').replace('{count}',String(summary.unvisited))}</span></div><p>{t('mentorSearchAssignedGroup')}</p><button onClick={()=>setPanel('people')}>{t('mentorOpenPeopleGardens')}</button></section>
      </>}
      {visitOpen&&<section ref={visitForm} className={styles.card} style={{margin:'16px 0'}} aria-label={t('mentorVisitDetailsAria')}>
        <div className={styles.row}><h2>{readOnlyVisit?paired('mentorFieldVisitDetailsHeading'):data.visits.some(v=>v.id===visit.id)?paired('mentorEditFieldVisit'):paired('mentorRecordFieldVisit')}</h2><button type="button" disabled={busy||captureBusy} onClick={closeVisit}>{paired('mentorCloseVisit')}</button></div>
        {photosBusy&&<p role="status">{paired('mentorPreparingVisitPhotos')}</p>}
        {!photosReady&&!photosBusy&&<p role="alert">{paired('mentorVisitPhotosLoadFailed')}</p>}
        <form onSubmit={e=>{e.preventDefault();void save(false);}}>
          <fieldset disabled={readOnlyVisit||busy||photosBusy||captureBusy||!photosReady} style={{border:0,padding:0,minWidth:0}}>
            ...
            <fieldset className={workStyles.focus}><legend>{paired('mentorSupportAreasCovered')}</legend><div>{Object.entries(FIELD_FOCUS).map(([key])=><label key={key}><input type="checkbox" checked={visit.focus?.includes(key as FieldFocus)??false} onChange={e=>setVisit({...visit,focus:e.target.checked?[...(visit.focus??[]),key as FieldFocus]:(visit.focus??[]).filter(f=>f!==key)})}/>{paired(FIELD_FOCUS_KEYS[key as FieldFocus])}</label>)}</div></fieldset>
            <label>{paired('mentorSupportRequestedLabel')}<textarea maxLength={2000} value={visit.supportRequested??''} placeholder={t('mentorSupportRequestedPlaceholder')} onChange={e=>setVisit({...visit,supportRequested:e.target.value})}/></label>
            <label>{paired('mentorObservationsLabel')}<textarea required={!visit.notes.trim()} maxLength={4000} value={visit.observations??''} placeholder={t('mentorObservationsPlaceholder')} onChange={e=>setVisit({...visit,observations:e.target.value})}/></label>
            <label>{paired('mentorAgreedActionLabel')}<textarea maxLength={2000} value={visit.agreedAction??''} placeholder={t('mentorAgreedActionPlaceholder')} onChange={e=>setVisit({...visit,agreedAction:e.target.value})}/></label>
            <div className={styles.grid}>
              <label>{paired('mentorResponsiblePersonLabel')}<input maxLength={120} value={visit.responsiblePerson??''} placeholder={t('mentorResponsiblePersonPlaceholder')} onChange={e=>setVisit({...visit,responsiblePerson:e.target.value})}/></label>
              <label>{paired('mentorFollowUpDateLabel')}<input type="date" min={visit.date} value={visit.followUpDate??''} onChange={e=>setVisit({...visit,followUpDate:e.target.value})}/></label>
            </div>
            <details className={workStyles.additional}><summary>{paired('mentorPracticalSkillCheck')}</summary><p>{paired('mentorPracticalSkillIntro')}</p><label>{paired('mentorSkillObserved')}<input maxLength={240} value={visit.practicalSkill??''} placeholder={t('mentorSkillObservedPlaceholder')} onChange={e=>setVisit({...visit,practicalSkill:e.target.value})}/></label><label>{paired('mentorObservationResultLabel')}<select value={visit.skillResult||'not-assessed'} onChange={e=>setVisit({...visit,skillResult:e.target.value as FieldVisit['skillResult']})}><option value="not-assessed">{t('mentorSkillNotAssessed')}</option><option value="demonstrated">{t('mentorSkillDemonstrated')}</option><option value="needs-support">{t('mentorSkillNeedsSupport')}</option></select></label></details>
            {visit.agreedAction?.trim()&&<details className={workStyles.additional} open={!!visit.actionCompletedOn}><summary>{paired('mentorCloseOutAction')}</summary><p>{paired('mentorCloseOutIntro')}</p><label>{paired('mentorCompletedOnLabel')}<input type="date" min={visit.date} max={today} value={visit.actionCompletedOn??''} onChange={e=>setVisit({...visit,actionCompletedOn:e.target.value})}/></label><label>{paired('mentorCompletionEvidenceLabel')}<textarea required={!!visit.actionCompletedOn} maxLength={2000} value={visit.actionOutcome??''} placeholder={t('mentorCompletionEvidencePlaceholder')} onChange={e=>setVisit({...visit,actionOutcome:e.target.value})}/></label></details>}
            ...
            {visit.originalNotes&&<details><summary>{paired('mentorOriginalNotesSummary')}</summary><p style={{whiteSpace:'pre-wrap'}}>{visit.originalNotes}</p></details>}
            {!readOnlyVisit&&<p className={styles.muted}>{paired('mentorVisitNotesVisibility')}</p>}
            {!readOnlyVisit&&<button className={styles.primary} disabled={busy||photosBusy||captureBusy||!photosReady}>{busy?paired('mentorSavingEllipsis'):paired('mentorSaveVisit')}</button>}
          </fieldset>
        </form>
        {/* Exported/copied report content stays English on purpose — goes to English-speaking funders (see programmeLabel above). */}
        {visitReport&&<><p className={styles.muted}>{t('mentorReportUsesSavedVisit')}</p><ReportComposer deviceData={data} title="Field visit report" sample={data.sample} orgId={org||undefined} photos={visitReport.photos??[]} photoHeading="Visit photographs" sections={[{title:'Visit details',lines:fieldVisitReportLines(visitReport,name(visitReport.farmerId),name(visitReport.mentorId))}]}/></>}
      </section>}
      {panel==='visits'&&scopeValid&&<section className={styles.card} style={{margin:'16px 0'}} aria-label={t('mentorRecordedVisitsAria')}>
        <h2>{t('mentorRecordedVisitsHeading')}</h2><p className={styles.muted}>{scopeLabel}</p>
        {!recordedVisits.length&&<p>{t('mentorNoVisitsRecorded')}</p>}
        {recordedVisits.slice(0,visibleCount).map(record=><article key={record.id} className={styles.metric}><div className={workStyles.actionHeader}><div className={workStyles.personHeading}>{portrait(record.farmerId)}<div><h3 style={{marginBottom:0}}>{name(record.farmerId)}</h3><p className={workStyles.personGarden}>{garden(record.farmerId)}</p></div></div><span className={styles.tag}>{record.date}</span></div><p>{record.observations||record.notes}</p>{record.agreedAction&&<p><strong>{record.actionCompletedOn?t('mentorCompletedActionLabel'):t('mentorNextActionLabel')}</strong> {record.agreedAction}<br/><strong>{t('mentorResponsibleLabel')}</strong> {record.responsiblePerson||t('mentorNotYetAssigned')} · <strong>{t('mentorFollowUpLabel')}</strong> {record.followUpDate||t('mentorNotYetScheduled')}{record.actionCompletedOn&&<><br/><strong>{t('mentorCompletedLabel')}</strong> {record.actionCompletedOn} · {record.actionOutcome}</>}</p>}{(record.photoCount??record.photos?.length??0)>0&&<p>{t('mentorVisitPhotosCount').replace('{count}',String(record.photoCount??record.photos?.length))}</p>}<button disabled={busy||photosBusy||captureBusy} onClick={()=>void openVisit(record)}>{canRecordVisits&&record.mentorId===data.selfId?t('mentorViewEditVisit'):t('mentorViewVisit')}</button></article>)}
        {recordedVisits.length>visibleCount&&<button onClick={()=>setVisibleCount(n=>n+10)}>{t('mentorShowMoreVisits').replace('{count}',String(recordedVisits.length-visibleCount))}</button>}
      </section>}
      {panel==='people'&&<><label>{t('mentorFindParticipantLabel')}<input type="search" value={peopleSearch} onChange={e=>setPeopleSearch(e.target.value)} placeholder={t('mentorSearchNamesGardens')}/></label><div className={workStyles.people}>
      {filteredPeople.map(person=>{const latest=data.visits.filter(v=>v.farmerId===person.id).sort((a,b)=>b.date.localeCompare(a.date))[0];return <article className={styles.card} key={person.id}><div className={workStyles.personHeading}>{portrait(person.id)}<h3>{person.name}</h3></div><p>{person.gardenName||data.teams.find(team=>team.farmerIds.includes(person.id))?.location||t('mentorGardenLocationTBD')}</p>{person.gardenAreaM2!=null&&<p className={styles.muted}>{person.gardenType} · {numberLabel(Math.round(person.gardenAreaM2))} m²</p>}<p className={styles.muted}>{latest?`${data.visitCursor?t('mentorMostRecentLoadedVisit'):t('mentorLastRecordedVisit')} · ${latest.date}`:data.visitCursor?t('mentorNoVisitInHistoryLoaded'):t('mentorNoVisitRecordedYet')}</p><div className={styles.row}>{canRecordVisits&&<button disabled={busy||captureBusy||photosBusy} onClick={()=>newVisit(person.id)}>{t('mentorRecordVisitButton')}</button>}{latest&&<button disabled={busy||captureBusy||photosBusy} onClick={()=>void openVisit(latest)}>{t('mentorLatestVisitButton')}</button>}</div></article>;})}
      {!filteredPeople.length&&<p>{t('mentorNoParticipantsMatch')}</p>}</div></>}
      {!data.teams.length && <p className={styles.card}>{t('mentorNoFieldTeamAssigned')}</p>}
      {(panel==='people'||organisation)&&data.teams.map(t => <article key={t.mentorId} className={styles.card} style={{ marginBottom: 16 }}><h2>{t.location}</h2><p style={{ display: 'flex', alignItems: 'center', gap: 12 }}>{portrait(t.mentorId,64)}<span>{translate(lang,'mentorLabelPrefix')} {name(t.mentorId)} · {translate(lang,'mentorUpdatedPrefix')} {t.updatedAt.slice(0, 10)}</span></p><h3>{translate(lang,'mentorOrganisationGuidanceHeading')}</h3><p style={{ whiteSpace: 'pre-wrap' }}>{t.guidance || translate(lang,'mentorNoGuidancePosted')}</p><p>{translate(lang,'mentorAssignedParticipantsCount').replace('{count}',String(t.farmerIds.length))} · {ui(FIELD_PROGRAMMES[t.programme??'general'],FIELD_PROGRAMMES_ZU[t.programme??'general'])}</p>{data.canManage && <button onClick={() => setDraft(t)}>{translate(lang,'mentorEditTeamGuidance')}</button>}</article>)}
      {data.canManage && <form className={styles.card} onSubmit={e => { e.preventDefault(); void save(true); }}><h2>{t('mentorAssignUpdateTeam')}</h2><label>{t('mentorMentorLabel')}<select required value={draft.mentorId} onChange={e => setDraft(data.teams.find(t => t.mentorId === e.target.value) ?? { ...draft, mentorId: e.target.value, farmerIds: [], guidance: '', location: '' })}><option value="">{t('mentorChooseMentorOption')}</option>{data.people.filter(p => p.role === 'mentor').map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>{t('mentorServiceLocationLabel')}<input required maxLength={160} value={draft.location} onChange={e => setDraft({ ...draft, location: e.target.value })} /></label><label>{t('mentorProgrammeFocusLabel')}<select value={draft.programme??'general'} onChange={e=>setDraft({...draft,programme:e.target.value as FieldProgramme})}>{Object.entries(FIELD_PROGRAMMES).map(([key,label])=><option key={key} value={key}>{ui(label,FIELD_PROGRAMMES_ZU[key as FieldProgramme])}</option>)}</select></label><h3>{t('mentorParticipantGroupHeading')}</h3><div className={styles.scroll}>{data.people.filter(p => ['farmer', 'student'].includes(p.role)).map(p => <label key={p.id} className={styles.option}><input type="checkbox" checked={draft.farmerIds.includes(p.id)} onChange={e => setDraft({ ...draft, farmerIds: e.target.checked ? [...draft.farmerIds, p.id] : draft.farmerIds.filter(id => id !== p.id) })} />{p.name}</label>)}</div><label>{t('mentorGuidanceForMentorLabel')}<textarea maxLength={4000} value={draft.guidance} onChange={e => setDraft({ ...draft, guidance: e.target.value })} /></label><button className={styles.primary} disabled={busy}>{t('mentorSaveTeamGuidance')}</button></form>}
      {/* Exported/copied report content (ReportComposer's title/photoHeading/metrics/chart/sections) stays English on purpose — goes to English-speaking funders, same decision as programmeLabel above. Only the screen note above it is translated. */}
      {panel==='report'&&summary&&scopeValid&&!data.visitCursor&&<><p className={styles.notice}>{t('mentorInternalReportNotice')}</p><ReportComposer deviceData={data} orgId={org||undefined} photos={summary.visits.flatMap(v=>(visitPhotos[v.id]??v.photos??[]).map(p=>({image:p.image,caption:`${v.date} · ${name(v.farmerId)} · ${p.caption}`})))} photoHeading="Visit photographs opened for this report" title={organisation ? 'Field implementation report' : 'Mentor field report'} sample={data.sample}
