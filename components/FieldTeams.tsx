    ...
    {data && <>
      ...
      {panel==='overview'&&summary&&scopeValid&&!data.visitCursor&&<>
        <div className={workStyles.metrics}>{[
          ['Assigned participants',summary.assigned.length,'Current field team'],
          ['Participants visited',`${summary.visited} / ${summary.assigned.length}`,'Within the selected period'],
          ['Follow-ups due',summary.due.length,'Open actions due by today'],
          ['Visits recorded',summary.visits.length,'Within the selected period'],
        ].map(([label,value,detail])=><div key={label} className={workStyles.metric}><span>{label}</span><strong>{value}</strong><small>{detail}</small></div>)}</div>
        <div className={workStyles.columns}>
          <section className={styles.card}><div className={styles.row}><h2>What needs attention</h2><span className={styles.tag}>{summary.open.length} open actions</span></div>
            ...
            {[...summary.due,...summary.open.filter(v=>!summary.due.includes(v))].slice(0,4).map(record=><article key={record.id} className={workStyles.action}><div className={styles.row}><strong>{name(record.farmerId)}</strong><span className={record.followUpDate&&record.followUpDate<=today?workStyles.due:styles.tag}>{actionLabel(record)}</span></div><p>{record.agreedAction}</p><small>{record.responsiblePerson||'Owner needed'} · {record.followUpDate||'Agree a follow-up date'}</small><button disabled={busy||captureBusy||photosBusy} onClick={()=>void openVisit(record)}>Review action</button></article>)}
            {summary.open.length>4&&<button onClick={()=>setPanel('visits')}>View all visit actions</button>}
          </section>
          <aside className={styles.card}><h2>{actProgramme?'Food-security visit guide':'A useful garden visit'}</h2><p className={styles.muted}>Use these prompts with the participant, then record what you observed.</p><ol className={workStyles.guide}><li><strong>Garden & water</strong><span>Check planted beds, water access, repairs and crop protection.</span></li><li><strong>Soil & seedlings</strong><span>Review soil cover, compost and the next planting.</span></li><li><strong>Harvest & food access</strong><span>Keep kilograms harvested, eaten or shared, sold and stored distinct.</span></li><li><strong>Learning & next action</strong><span>Ask for a practical demonstration. Agree an action, owner and date.</span></li></ol>
          <p className={styles.muted}>{actProgramme?'These records support ACT’s SEF food-security delivery. Employment attendance and payroll remain in the approved SEF process.':'Training attendance and observed practical skills are recorded separately.'}</p></aside>
        </div>
        <section className={styles.card}><div className={styles.row}><h2>Plan the next garden visit</h2><span className={styles.tag}>{summary.unvisited} participants without a visit in this period</span></div><p>Search your assigned group, open their recent records, or start a visit with their name already selected.</p><button onClick={()=>setPanel('people')}>Open people & gardens</button></section>
      </>}
