...
export default function FieldTeams({ organisation = false }: { organisation?: boolean }) {
  ...
  const [visit, setVisit] = useState({ id: '', farmerId: '', date: '', notes: '' }), [notice, setNotice] = useState('');
  ...
  async function save(team: boolean) {
    ...
    try {
      ...
      setVisit({ id: '', farmerId: '', date: '', notes: '' }); await reload();
