...
export default function FieldTeams({ organisation = false, initialFarmerId, onStartTraining }: { organisation?: boolean; initialFarmerId?: string; onStartTraining?:()=>void }) {
  ...
  return <section className={`${styles.root} ${workStyles.workspace}`}><div className={styles.wrap}>
    {lang === 'zu' && <p role="note" className={styles.notice}>{ui('The isiZulu wording in this visit workspace is an unreviewed draft. English is shown beside location, privacy, assessment and save meanings. Your notes and names stay exactly as you entered them.', 'Umbhalo wesiZulu kule ndawo yamanothi okuvakasha uwuhlaka olungakabuyekezwa. IsiNgisi siboniswa eduze kwencazelo yendawo, ubumfihlo, ukuhlola nokugcina. Amanothi namagama akho ahlala njengoba uwafakile.')}</p>}
    ...
    <div className={workStyles.hero}>
      <div><p className={workStyles.eyebrow}>{organisation?ui('Programme coordination','Ukuhlelwa kohlelo'):programmeLabelUi}</p><h1>{organisation?ui('Mentor teams & guidance','Amaqembu abeluleki nesiqondiso'):ui('My fieldwork','Umsebenzi wami wasensimini')}</h1><p>{organisation?ui('Give each mentor a clear group, programme focus and current instructions.','Nikeza umeluleki ngamunye iqembu elicacile, okugxilwe kukho ohlelweni nemiyalelo esebenzayo.'):ui('Support the garden. Build practical skills. Follow through on agreed actions.','Sekela ingadi. Yakha amakhono awusizo. Landela izinyathelo enivumelene ngazo.')}</p></div>
      {!organisation&&<div className={workStyles.heroActions}><button className={styles.primary} disabled={!canRecordVisits||busy||photosBusy||captureBusy} onClick={()=>newVisit()}>{ui('Record a field visit','Rekhoda ukuvakasha ensimini')}</button>{onStartTraining&&<button onClick={onStartTraining}>{ui('Training & attendance','Ukuqeqeshwa nokuba khona')}</button>}</div>}
    </div>
    ...
    {data && <>
      <nav className={workStyles.tabs} aria-label={ui('Fieldwork views','Ukubuka umsebenzi wasensimini')}>{([['overview','Priorities','Okubalulekile'],['people','People & gardens','Abantu nezingadi'],['visits','Visit records','Amarekhodi okuvakasha'],['report','Field report','Umbiko wasensimini']] as const).map(([key,label,zu])=><button key={key} aria-pressed={panel===key} onClick={()=>setPanel(key)}>{ui(label,zu)}</button>)}</nav>
      ...
      {visitOpen&&<section ref={visitForm} className={styles.card} style={{margin:'16px 0'}} aria-label={ui('Visit details','Imininingwane yokuvakasha')}>
        <div className={styles.row}><h2>{readOnlyVisit?ui('Field visit details','Imininingwane yokuvakasha ensimini'):data.visits.some(v=>v.id===visit.id)?ui('Edit field visit','Hlela ukuvakasha ensimini'):ui('Record a field visit','Rekhoda ukuvakasha ensimini')}</h2><button type="button" disabled={busy||captureBusy} onClick={closeVisit}>{ui('Close visit','Vala ukuvakasha')}</button></div>
        {photosBusy&&<p role="status">{ui('Preparing visit photos…','Kulungiselelwa izithombe zokuvakasha…')}</p>}
        {!photosReady&&!photosBusy&&<p role="alert">{ui('Visit photos could not be loaded. Close and reopen this visit before editing it.','Izithombe zokuvakasha azikwazanga ukulayishwa. Vala uphinde uvule lokhu kuvakasha ngaphambi kokukuhlela.')}</p>}
        <form onSubmit={e=>{e.preventDefault();void save(false);}}>
          <fieldset disabled={readOnlyVisit||busy||photosBusy||captureBusy||!photosReady} style={{border:0,padding:0,minWidth:0}}>
            <div className={styles.grid}>
              <label>{ui('Participant','Umbambiqhaza')}<select required value={visit.farmerId} onChange={e=>setVisit({...visit,farmerId:e.target.value,location:data.people.find(p=>p.id===e.target.value)?.gardenName??'',latitude:null,longitude:null})}><option value="">{ui('Choose a participant','Khetha umbambiqhaza')}</option>{[...new Set([...assigned,...(visit.farmerId?[visit.farmerId]:[])])].map(id=><option key={id} value={id}>{name(id)}</option>)}</select></label>
              <label>{ui('Visit date','Usuku lokuvakasha')}<input required type="date" max={new Date().toISOString().slice(0,10)} value={visit.date} onChange={e=>setVisit({...visit,date:e.target.value})}/></label>
            </div>
            <label>{ui('Site location (optional)','Indawo yesayithi (uma uthanda)')}<input maxLength={240} value={visit.location??''} placeholder={ui('Garden name, meeting point or directions','Igama lengadi, indawo yokuhlangana noma izikhombisi-ndlela')} onChange={e=>setVisit({...visit,location:e.target.value})}/></label>
            <details><summary>{ui('Location, map & directions','Indawo, imephu nezikhombisi-ndlela')}</summary><VenueLocation key={`${visit.id}|${visit.farmerId}`} latitude={visit.latitude??null} longitude={visit.longitude??null} sample={data.sample} recordKind="visit" venue={visit.location} onChange={point=>{if(captureScope===currentScope()&&captureVersion===requestVersion.current)setVisit(current=>({...current,...point}));}}/></details>
            <fieldset className={workStyles.focus}><legend>{ui('Support areas covered','Izindawo zosizo ezihlanganisiwe')}</legend><div>{Object.entries(FIELD_FOCUS).map(([key,label])=><label key={key}><input type="checkbox" checked={visit.focus?.includes(key as FieldFocus)??false} onChange={e=>setVisit({...visit,focus:e.target.checked?[...(visit.focus??[]),key as FieldFocus]:(visit.focus??[]).filter(f=>f!==key)})}/>{ui(label, ({garden:'Ukusungula nokunakekela ingadi',water:'Ukutholakala kwamanzi nokulungisa',soil:'Impilo yomhlabathi nomquba',seedlings:'Izithombo nokugcina imbewu',crops:'Ukunakekelwa nokuvikelwa kwezitshalo',harvest:'Ukuvuna nokwabelana ngokudla',learning:'Amakhono awusizo nokuqeqesha',records:'Amarekhodi nokusekela indlela yokuziphilisa'} as Record<string,string>)[key] ?? label)}</label>)}</div></fieldset>
            <label>{ui('Support requested','Usizo oluceliwe')}<textarea maxLength={2000} value={visit.supportRequested??''} placeholder={ui('Water, tools, inputs, crop care, records or other support requested','Amanzi, amathuluzi, izinto zokufaka, ukunakekelwa kwezitshalo, amarekhodi noma olunye usizo oluceliwe')} onChange={e=>setVisit({...visit,supportRequested:e.target.value})}/></label>
            <label>{ui('Observations and issues','Okubonile nezinkinga')}<textarea required={!visit.notes.trim()} maxLength={4000} value={visit.observations??''} placeholder={ui('Describe the garden condition and what you checked. Include quantities and units when measured.','Chaza isimo sengadi nokuhlolile. Faka amanani namayunithi uma ulinganisile.')} onChange={e=>setVisit({...visit,observations:e.target.value})}/></label>
            <label>{ui('Agreed action','Isinyathelo okuvunyelwene ngaso')}<textarea maxLength={2000} value={visit.agreedAction??''} placeholder={ui('What will happen next?','Kuzokwenzekani ngokulandelayo?')} onChange={e=>setVisit({...visit,agreedAction:e.target.value})}/></label>
            <div className={styles.grid}>
              <label>{ui('Responsible person','Umuntu onesibopho')}<input maxLength={120} value={visit.responsiblePerson??''} placeholder={ui('Who will carry out the action?','Ubani ozokwenza lesi sinyathelo?')} onChange={e=>setVisit({...visit,responsiblePerson:e.target.value})}/></label>
              <label>{ui('Follow-up date','Usuku lokulandelela')}<input type="date" min={visit.date} value={visit.followUpDate??''} onChange={e=>setVisit({...visit,followUpDate:e.target.value})}/></label>
            </div>
            <details className={workStyles.additional}><summary>{ui('Practical skill check','Ukuhlola ikhono elisebenzayo')}</summary><p>{ui('Record a skill demonstrated in the garden separately from course completion or attendance.','Rekhoda ikhono eliboniswe engadini ngokwehlukana nokuqeda isifundo noma ukuba khona ekuqeqeshweni.')}</p><label>{ui('Skill observed','Ikhono elibonwe')}<input maxLength={240} value={visit.practicalSkill??''} placeholder={ui('e.g. weigh and record a harvest','isib. kala bese ubhala isivuno')} onChange={e=>setVisit({...visit,practicalSkill:e.target.value})}/></label><label>{ui('Observation result','Umphumela wokubheka')}<select value={visit.skillResult||'not-assessed'} onChange={e=>setVisit({...visit,skillResult:e.target.value as FieldVisit['skillResult']})}><option value="not-assessed">{ui('Not assessed','Akukahlolwa')}</option><option value="demonstrated">{ui('Demonstrated during this visit','Kuboniswe kulokhu kuvakasha')}</option><option value="needs-support">{ui('Needs further support','Kudinga olunye usizo')}</option></select></label></details>
            {visit.agreedAction?.trim()&&<details className={workStyles.additional} open={!!visit.actionCompletedOn}><summary>{ui('Close out this action','Qinisekisa ukuqedwa kwalesi sinyathelo')}</summary><p>{ui('Mark completion only after checking what changed. Leave the date blank while the action is open.','Maka ukuthi kuqediwe kuphela ngemva kokuhlola okushintshile. Shiya usuku lungenalutho uma isinyathelo sisavuliwe.')}</p><label>{ui('Completed on','Kuqedwe ngomhla')}<input type="date" min={visit.date} max={today} value={visit.actionCompletedOn??''} onChange={e=>setVisit({...visit,actionCompletedOn:e.target.value})}/></label><label>{ui('Completion evidence','Ubufakazi bokuqedwa')}<textarea required={!!visit.actionCompletedOn} maxLength={2000} value={visit.actionOutcome??''} placeholder={ui('What was completed, and how did you check it?','Yini eqediwe, futhi uyihlole kanjani?')} onChange={e=>setVisit({...visit,actionOutcome:e.target.value})}/></label></details>}
            ...
            {visit.originalNotes&&<details><summary>{ui('Original notes before AI cleanup','Amanothi okuqala ngaphambi kokulungiswa nge-AI')}</summary><p style={{whiteSpace:'pre-wrap'}}>{visit.originalNotes}</p></details>}
            {!readOnlyVisit&&<p className={styles.muted}>{ui('Visit notes and photos are visible to the assigned mentor and authorised organisation staff.','Amanothi okuvakasha nezithombe zibonwa umeluleki owabelwe nabasebenzi benhlangano abagunyaziwe.')}</p>}
            {!readOnlyVisit&&<button className={styles.primary} disabled={busy||photosBusy||captureBusy||!photosReady}>{busy?ui('Saving…','Kuyagcinwa…'):ui('Save visit','Gcina ukuvakasha')}</button>}
          </fieldset>
        </form>
