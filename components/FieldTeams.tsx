...
export default function FieldTeams({ organisation = false }: { organisation?: boolean }) {
  ...
  return <section className={styles.root}><div className={styles.wrap}>
    ...
    {data && <>{data.sample && <p className={styles.notice}>Fictional practice workspace. Changes stay in this sample. No notifications are sent.</p>}
      ...
      {data.teams.map(t => <article key={t.mentorId} className={styles.card} style={{ marginBottom: 16 }}><h2>{t.location}</h2><p>Mentor: {name(t.mentorId)} · updated {t.updatedAt.slice(0, 10)}</p><h3>Organisation guidance</h3><p style={{ whiteSpace: 'pre-wrap' }}>{t.guidance || 'No guidance posted yet.'}</p><h3>Assigned farmers and learners</h3>{t.farmerIds.map(id => <p key={id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>{data.sample && <img src={samplePortrait(id)} alt="Fictional profile portrait" width={52} height={52} style={{ borderRadius: '50%' }} />}{name(id)}</p>)}{data.canManage && <button onClick={() => setDraft(t)}>Edit team & guidance</button>}</article>)}
