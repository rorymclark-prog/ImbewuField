#!/usr/bin/env python3
"""Stage Vegetables and Staple Crops isiZulu review audio from its source-paired drafts.

Run: uv run --with edge-tts python scripts/record-vegetables-staples-zu-draft.py [stage-dir]
...
DEFAULT_STAGE = Path.home() / "Downloads/imbewu-record/vegetables-staples-zu-20260924"
...
async def main() -> int:
    ...
    script = REPO / "docs/narration/vegetables-staples.zu.md"
    english = REPO / "docs/narration/vegetables-staples.en.md"
    ...
    expected_all = [f"slide-{i:02d}.txt" for i in range(1, 19)]
    if [path.name for path in sources] != expected_all:
        raise SystemExit(f"expected 18 exported blocks; found {[p.name for p in sources]}")
    ...
    for source in selected:
        ...
        lesson = "vegetables-staples-l1" if slide <= 7 else ("vegetables-staples-l2" if slide <= 11 else ("vegetables-staples-l3" if slide <= 14 else ("vegetables-staples-l4" if slide <= 16 else None)))
    ...
    full_proof = {"module": MODULE, "lesson": None, "scope": "slides-1-through-18",
    ...
    (stage / "RUN.json").write_text(json.dumps(run_record, ensure_ascii=False, indent=2) + "\n",
                                     encoding="utf-8")
    table = ["# Vegetables and Staple Crops isiZulu review audio — asset sizes", "",
