#!/usr/bin/env python3
"""Stage Food Forest Design isiZulu review audio from its source-paired drafts.

Run: uv run --with edge-tts python scripts/record-food-forest-zu-draft.py [stage-dir]
...
DEFAULT_STAGE = Path.home() / "Downloads/imbewu-record/food-forest-zu-20260924"
MODULE = "food-forest"
LANGUAGE = "zu"
...
async def main() -> int:
    ...
    script = REPO / "docs/narration/food-forest.zu.md"
    english = REPO / "docs/narration/food-forest.en.md"
    ...
    for source in selected:
        ...
        lesson = ("food-forest-l1" if 3 <= slide <= 8 else ("food-forest-l2" if 9 <= slide <= 13 else ("food-forest-l3" if 14 <= slide <= 18 else None)))
    ...
    (stage / "RUN.json").write_text(json.dumps(run_record, ensure_ascii=False, indent=2) + "\n",
                                     encoding="utf-8")
    table = ["# Food Forest Design isiZulu review audio — asset sizes", "",
