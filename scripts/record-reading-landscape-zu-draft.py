...
Run: uv run --with edge-tts python scripts/record-reading-landscape-zu-draft.py [stage-dir]
...
DEFAULT_STAGE = Path.home() / "Downloads/imbewu-record/reading-landscape-zu-20260924"
...
async def main() -> int:
    ...
    script = REPO / "docs/narration/reading-landscape.zu.md"
    english = REPO / "docs/narration/reading-landscape.en.md"
    ...
    expected_all = [f"slide-{i:02d}.txt" for i in range(1, 22)]
    ...
    for source in selected:
        ...
        lesson = ("reading-landscape-l1" if 4 <= slide <= 7 else ("reading-landscape-l2" if 8 <= slide <= 11 else ("reading-landscape-l3" if 12 <= slide <= 15 else ("reading-landscape-l4" if 16 <= slide <= 19 else None))))
    ...
    (stage / "RUN.json").write_text(json.dumps(run_record, ensure_ascii=False, indent=2) + "\n",
                                     encoding="utf-8")
    table = ["# Reading the Landscape isiZulu review audio — asset sizes", "",
