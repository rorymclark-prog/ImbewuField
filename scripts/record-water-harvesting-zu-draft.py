#!/usr/bin/env python3
"""Stage Reading the Landscape isiZulu review audio from its source-paired drafts.

Run: uv run --with edge-tts python scripts/record-water-harvesting-zu-draft.py [stage-dir]
...
DEFAULT_STAGE = Path.home() / "Downloads/imbewu-record/water-harvesting-zu-20260924"
MODULE = "water-harvesting"
LANGUAGE = "zu"
...
async def main() -> int:
    ...
    script = REPO / "docs/narration/water-harvesting.zu.md"
    english = REPO / "docs/narration/water-harvesting.en.md"
    ...
    expected_all = [f"slide-{i:02d}.txt" for i in range(1, 25)]
    if [path.name for path in sources] != expected_all:
        raise SystemExit(f"expected 21 exported blocks; found {[p.name for p in sources]}")
    ...
    for source in selected:
        ...
        lesson = ("water-harvesting-l1" if 3 <= slide <= 9 else ("water-harvesting-l2" if 10 <= slide <= 13 else ("water-harvesting-l3" if 14 <= slide <= 18 else ("water-harvesting-l4" if 19 <= slide <= 22 else None))))
    ...
    full_proof = {"module": MODULE, "lesson": None, "scope": "slides-1-through-21",
    ...
    (stage / "RUN.json").write_text(json.dumps(run_record, ensure_ascii=False, indent=2) + "\n",
                                     encoding="utf-8")
    table = ["# Water Harvesting isiZulu review audio — asset sizes", "",
