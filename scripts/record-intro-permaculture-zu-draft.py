#!/usr/bin/env python3
"""Stage Introduction isiZulu review audio from its source-paired drafts.

Run: uv run --with edge-tts python scripts/record-intro-l1-zu-draft.py [stage-dir]
...
DEFAULT_STAGE = Path.home() / "Downloads/imbewu-record/intro-permaculture-l1-zu-20260924"
MODULE = "intro-permaculture"
LANGUAGE = "zu"
...
async def main() -> int:
    ...
    script = REPO / "docs/narration/intro-permaculture.zu.md"
    english = REPO / "docs/narration/intro-permaculture.en.md"
    ...
    expected_all = [f"slide-{i:02d}.txt" for i in range(1, 23)]
    if [path.name for path in sources] != expected_all:
        raise SystemExit(f"expected 22 exported blocks; found {[p.name for p in sources]}")
    ...
    for source in selected:
        ...
        lesson = "intro-permaculture-l1" if slide <= 8 else ("intro-permaculture-l2" if slide <= 14 else "intro-permaculture-l3")
    ...
    full_proof = {"module": MODULE, "lesson": None, "scope": "slides-1-through-22",
    ...
    (stage / "RUN.json").write_text(json.dumps(run_record, ensure_ascii=False, indent=2) + "\n",
                                     encoding="utf-8")
    table = ["# Introduction isiZulu review audio — asset sizes", "",
