...
const illustratedCounts=data.map(d=>`${d.id.replaceAll('-',' ')}: ${d.languages.en.filter(b=>b.number>1&&b.frames.some(f=>f.kind==='illustration')).length}/${d.languages.en.length-1}`);
...
<header><h1>Imbewu lesson deck review</h1><p class="status">Production draft. English: nine reading decks with covers. Illustrated teaching fronts after each cover: ${illustratedCounts.join('; ')}. Soil Health's wattle-pod identification image remains pending. isiZulu: all nine reading decks are drafts requiring reconciliation and first-language review. This file contains stills only; animations and narration remain separate.</p><div class="controls"><label>Module <select id="module"></select></label><label>Language <select id="language"><option value="en">English</option><option value="zu">isiZulu · DRAFT</option></select></label></div></header>
