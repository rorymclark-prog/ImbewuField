...
export default function SampleGardensPage() {
  ...
  return <main className={styles.page}><div className={styles.wrap}>
    ...
    {garden&&<section ref={detail} className={styles.card} aria-label={ui('Selected garden','Ingadi ekhethiwe')}>
      ...
      <h2 aria-live="polite">{garden.name}</h2><p>{garden.town} · {kindLabel(garden.kind)} · {Math.round(garden.areaM2??0).toLocaleString()} m² {ui('illustrative area','indawo eyisibonelo')}</p>
      <SampleGardenVisual key={garden.id} name={garden.name} kind={garden.kind} variant={garden.id} initial="aerial"/>
      <div className={styles.grid} style={{marginTop:20}}>{[[ui('Site area','Indawo yengadi'),garden.areaM2 ?? 0],[ui('Vegetable beds','Imibhede yemifino'),garden.production.vegetableM2],[ui('Staple plots','Iziza zezitshalo eziyinhloko'),garden.production.stapleM2]].map(([label,value])=><section className={styles.card} key={label}><h3>{label}</h3><strong>{Number(value).toLocaleString()} m²</strong></section>)}</div><p>{ui('Growing areas exclude buildings, paths, tree areas and unused space. Photos illustrate the setting; they do not measure the land.','Izindawo zokutshala azifaki izakhiwo, izindlela, izindawo zezihlahla noma umhlaba ongasetshenziswa. Izithombe zibonisa indawo kuphela; azilinganisi umhlaba.')}</p>
