...
export default function SampleGardensPage() {
  ...
  return <main className={styles.page}><div className={styles.wrap}>
    ...
    <h1>{SAMPLE_GARDENS.length} gardens</h1><p>Choose a garden to explore its layout, people and report.</p>
    {garden&&<section ref={detail} className={styles.card} aria-label="Selected garden">
      ...
      <div className={styles.grid} style={{marginTop:20}}>{[['Site area',garden.areaM2 ?? 0],['Vegetable beds',garden.production.vegetableM2],['Staple plots',garden.production.stapleM2]].map(([label,value])=><section className={styles.card} key={label}><h3>{label}</h3><strong>{Number(value).toLocaleString()} m²</strong></section>)}</div><p>Growing areas exclude buildings, paths, tree areas and unused space. Photos illustrate the setting; they do not measure the land.</p>
      <h2 style={{marginTop:24}}>Participants</h2><p>{garden.language}-speaking group.</p>
      <div className={styles.grid}>{(SAMPLE_PARTICIPANTS[garden.language??'isiZulu']??[]).map(name=><figure key={name} style={{display:'flex',alignItems:'center',gap:14}}><img data-photo-preview src={samplePortrait(name)} alt={`Portrait illustration of ${name}`} width={80} height={80} style={{borderRadius:'50%',objectFit:'cover'}}/><figcaption>{name}</figcaption></figure>)}</div>
      <div className={styles.actions}><a href={sampleGardenReportUrl(garden.id)} target="_blank" rel="noreferrer">Open completed report (PDF)</a><a href={sampleGardenReportUrl(garden.id)} download={`${garden.id}-tour-report.pdf`}>Download report</a></div>
      <p>The saved report includes this garden's site photograph, layout, assessment narrative, programme figures and next actions. Generate a new report below to create another copy.</p>
