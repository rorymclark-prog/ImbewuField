...
export default function SampleGardensPage() {
  ...
  return <main className={styles.page}><div className={styles.wrap}>
    ...
    {garden&&<section ref={detail} className={styles.card} aria-label="Selected sample garden">
      ...
      <SampleGardenVisual key={garden.id} name={garden.name} kind={garden.kind} variant={garden.id}/>
      ...
      <ReportComposer key={`report-${garden.id}`} sample title={`${garden.name} — example report`} branding={{...SAMPLE_BRANDING,garden:{...SAMPLE_BRANDING.garden,label:garden.name}}} sections={[
        {title:'Garden',lines:[garden.name,`${garden.kind} · ${garden.town}`,`Illustrative site area: ${Math.round(garden.areaM2??0)} m²`, `Example group language: ${garden.language}`]},
        ...
        {title:'About this sample',lines:['All figures and people are fictional. The diagram is not a measured planting design. The shared reference photo does not depict this location. This overview is not a site assessment, soil test or bill of quantities.']},
      ]}/>
    ...
    <p>{visible.length} gardens shown</p><div className={styles.grid}>{visible.map(g=><button key={g.id} className={styles.card} onClick={()=>setSelected(g.id)} style={{display:'block',textAlign:'left',borderRadius:18,margin:0}} aria-label={`Open ${g.name}`}>
      <img src={sampleGardenImage(g.kind??'Community garden',g.id)} alt="" style={{width:'100%',borderRadius:12}}/><h2 style={{marginTop:12}}>{g.name}</h2><p>{g.kind} · {g.town}</p><span>Open garden →</span>
    </button>)}</div>
  </div></main>;
}
