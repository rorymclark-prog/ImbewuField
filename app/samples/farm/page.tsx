...
export default function SampleFarmPage() {
  ...
  async function report(){if(!pack||busy)return;setBusy(true);setMessage('');try{
    ...
    if(!isSampleMode())return;
    await deliverFile(pdf.output('blob'),'ImbewuField-SAMPLE-Farm-Evidence.pdf','Fictional sample farm evidence');setMessage('Report ready. Use your device’s save or share controls.');
  ...
  return <main className={styles.page}><div className={styles.wrap}><header className={styles.header}><MenuButton/><BackButton fallback="/samples"/><SettingsButton/><Link href="/tour">15-minute tour</Link></header>
    ...
    {!sample?<button className={styles.primary} onClick={()=>{if(!enterSampleMode())setMessage('Please allow session storage to open this sample.');}}>Open farm workspace</button>:pack&&<>
