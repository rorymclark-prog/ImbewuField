...
export default function SamplesPage() {
  ...
  return <main className={styles.root} style={{ height: '100dvh', minHeight: 0, overflowY: 'auto', paddingBottom: 'calc(var(--bottom-nav-height, 64px) + 24px + env(safe-area-inset-bottom, 0px))' }}><div className={styles.wrap}>
    ...
    <div className={styles.hero} style={{ marginTop: 16, padding:20 }}><span>IMBEWUFIELD · EXPLORE</span><h1>Choose a view</h1><p>Explore each workspace. Use Tour in the menu to return here.</p></div>
    {error && <p role="alert">{error}</p>}
    <section aria-label="All views" style={{marginBottom:24}}><h2>Choose your workspace</h2><p>{!choicesReady ? 'Checking available views…' : user && !accountRole ? 'Your account role could not be confirmed. Refresh your access in Account, then return here.' : accountRole && !['ngo','admin'].includes(accountRole) ? `Your ${accountRole} account opens its matching practice view. Other roles are shown below so the full set is visible.` : "Choose any of the five workspaces."}</p>
    <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}} aria-label="view index">{examples.map(([id,label])=><a key={id} href={`#sample-${id}`} style={{padding:'10px 12px',border:'1px solid var(--border)',borderRadius:20}}>{label}</a>)}</div>
    <div className={styles.sampleGrid}>{examples.map(([sampleRole, label, title, description]) => { const Icon = icons[sampleRole]; const allowed = availableRoles.includes(sampleRole); return <article id={`sample-${sampleRole}`} key={sampleRole} className={`${styles.card} ${styles.sampleCard}`}><Icon size={24} aria-hidden="true"/><span className={styles.tag}>{label}</span><h3 style={{margin:'8px 0'}}>{title}</h3><p>{description}</p><button type="button" disabled={!allowed} className={styles.primary} onClick={() => { if (!allowed) return; if (startRolePreview(sampleRole)) router.push('/' + sampleRole); else setError("tour mode could not start. Please allow session storage and try again."); }}>{allowed ? `Open ${label}` : !choicesReady ? 'Checking access…' : 'Not available for this account'}</button></article>; })}</div></section>
    <div className={styles.card}><h2>Browse 18 different gardens</h2><p>Homesteads, commercial plots, crèches, schools and community gardens. Choose one to see its layout, participants and report.</p><Link className={styles.primary} href="/samples/gardens">Browse 18 gardens →</Link></div>
    <div className={styles.card}><h2>Editable farm workspace · Ubhejane</h2><p>Ubhejane is currently the connected editable farm. The 18 garden profiles above are overview examples. Try the map and editable design, review a completed assessment, review the household interview and soil record, then download an evidence report.</p><div className={styles.row}><Link href="/tour">Start the 15-minute tour</Link><Link href="/samples/farm">Open the farm evidence pack</Link></div></div>
    <button type="button" onClick={() => { if (enterSampleMode()) window.location.reload(); }}>Reset tour records</button>
    <p>Changes stay in this tour session. Resetting or reloading starts fresh. Signing in is not required and switching views never changes your real account permissions.</p>
  </div></main>;
}
