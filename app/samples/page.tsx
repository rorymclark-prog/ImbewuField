...
export default function SamplesPage() {
  ...
  const examples = lang === 'zu' ? examplesZu : examplesEn;
  ...
  return <main className={styles.root} style={{ height: '100dvh', minHeight: 0, overflowY: 'auto', paddingBottom: 'calc(var(--bottom-nav-height, 64px) + 24px + env(safe-area-inset-bottom, 0px))' }}><div className={styles.wrap}>
    ...
    <div className={styles.hero} style={{ marginTop: 16, padding:20 }}><span>IMBEWUFIELD · {ui('EXPLORE', 'HLOLA')}</span><h1>{ui('Choose a view', 'Khetha indawo yokusebenza')}</h1><p>{ui('Explore each workspace. Use Tour in the menu to return here.', 'Hlola indawo ngayinye yokusebenza. Sebenzisa uHambo kumenyu ukuze ubuyele lapha.')}</p></div>
    {error && <p role="alert">{error}</p>}
    <section aria-label={ui('All views', 'Zonke izindawo')} style={{marginBottom:24}}><h2>{ui('Choose your workspace', 'Khetha indawo yakho yokusebenza')}</h2><p>{!choicesReady ? ui('Checking available views…', 'Kubhekwa izindawo ezitholakalayo…') : user && !accountRole ? ui('Your account role could not be confirmed. Refresh your access in Account, then return here.', 'Indima ye-akhawunti yakho ayiqinisekisiwe. Vuselela ukufinyelela kwakho ku-I-akhawunti, bese ubuya lapha.') : accountRole && !['ngo','admin'].includes(accountRole) ? ui(`Your ${accountRole} account opens its matching practice view. Other roles are shown below so the full set is visible.`, `I-akhawunti yakho ye-${accountRole} ivula indawo ehambisana nayo yokuzilolonga. Ezinye izindima ziboniswe ngezansi ukuze ubone zonke.`) : ui('Choose any of the five workspaces.', 'Khetha noma iyiphi yezindawo ezinhlanu zokusebenza.')}</p>
    <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}} aria-label={ui('View index', 'Uhlu lwezindawo')}>{examples.map(([id,label])=><a key={id} href={`#sample-${id}`} style={{padding:'10px 12px',border:'1px solid var(--border)',borderRadius:20}}>{label}</a>)}</div>
    <div className={styles.sampleGrid}>{examples.map(([sampleRole, label, title, description]) => { const Icon = icons[sampleRole]; const allowed = availableRoles.includes(sampleRole); return <article id={`sample-${sampleRole}`} key={sampleRole} className={`${styles.card} ${styles.sampleCard}`}><Icon size={24} aria-hidden="true"/><span className={styles.tag}>{label}</span><h3 style={{margin:'8px 0'}}>{title}</h3><p>{description}</p><button type="button" disabled={!allowed} className={styles.primary} onClick={() => { if (!allowed) return; if (startRolePreview(sampleRole)) router.push('/' + sampleRole); else setError(ui('Tour mode could not start. Please allow session storage and try again.', 'Uhambo alukwazanga ukuqala. Vumela ukugcinwa kolwazi lweseshini bese uzama futhi.')); }}>{allowed ? ui(`Open ${label}`, `Vula: ${label}`) : !choicesReady ? ui('Checking access…', 'Kubhekwa ukufinyelela…') : ui('Not available for this account', 'Akutholakali kule akhawunti')}</button></article>; })}</div></section>
