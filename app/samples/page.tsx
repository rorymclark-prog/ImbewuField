...
import { SESOTHO_SAMPLE_CHOOSER_DRAFT } from '@/lib/sample-chooser-translation-draft-st';
import { TSHIVENDA_SAMPLE_CHOOSER_DRAFT } from '@/lib/sample-chooser-translation-draft-ve';
...
function resolveChooserPair(pair: ChooserPair, lang: string) {
  if (pair.reviewStatus === 'hold') return pair.sourceEnglish;
  if (lang === 'st') return pair.sesothoDraft ?? pair.sourceEnglish;
  if (lang === 've') return pair.tshivendaDraft ?? pair.sourceEnglish;
  return pair.sourceEnglish;
}
function PairedDraft({ pair, lang, inline = false }: { pair: ChooserPair; lang: string; inline?: boolean }) {
  if (pair.reviewStatus === 'hold') return <>{pair.sourceEnglish}</>;
  return <>{resolveChooserPair(pair, lang)}<span style={{ display: inline ? 'inline' : 'block', fontSize: inline ? '0.72em' : '0.82em', fontWeight: 400, opacity: 0.78 }}> English: {pair.sourceEnglish}</span></>;
}
export default function SamplesPage() {
  const router = useRouter(); const [error, setError] = useState('');
  const { lang } = useLanguage();
  const ui = (english: string, zulu: string) => lang === 'zu' ? zulu : english;
  const draftText = (pair: ChooserPair) => resolveChooserPair(pair, lang);
  const regionalRoles = lang === 'st' ? SESOTHO_SAMPLE_CHOOSER_DRAFT.roles : TSHIVENDA_SAMPLE_CHOOSER_DRAFT.roles;
  const examples = lang === 'zu' ? examplesZu : (lang === 'st' || lang === 've')
    ? Object.entries(regionalRoles)
      .map(([id, copy]) => [id as keyof typeof icons, draftText(copy.label), draftText(copy.title), draftText(copy.subtitle)] as const)
    : examplesEn;
  ...
  return <main className={styles.root} style={{ height: '100dvh', minHeight: 0, overflowY: 'auto', paddingBottom: 'calc(var(--bottom-nav-height, 64px) + 24px + env(safe-area-inset-bottom, 0px))' }}><div className={styles.wrap}>
    ...
    <div className={styles.hero} style={{ marginTop: 16, padding:20 }}><span style={lang === 'st' || lang === 've' ? { display: 'block' } : undefined}>{lang === 'st' ? <>IMBEWUFIELD · <PairedDraft lang={lang} pair={SESOTHO_SAMPLE_CHOOSER_DRAFT.eyebrow} inline /></> : lang === 've' ? <>IMBEWUFIELD · <PairedDraft lang={lang} pair={TSHIVENDA_SAMPLE_CHOOSER_DRAFT.eyebrow} inline /></> : `IMBEWUFIELD · ${ui('EXPLORE', 'HLOLA')}`}</span><h1>{lang === 'st' ? <PairedDraft lang={lang} pair={SESOTHO_SAMPLE_CHOOSER_DRAFT.heading} /> : lang === 've' ? <PairedDraft lang={lang} pair={TSHIVENDA_SAMPLE_CHOOSER_DRAFT.heading} /> : ui('Choose a view', 'Khetha indawo yokusebenza')}</h1><p style={lang === 'st' || lang === 've' ? { display: 'block' } : undefined}>{lang === 'st' ? <PairedDraft lang={lang} pair={SESOTHO_SAMPLE_CHOOSER_DRAFT.intro} /> : lang === 've' ? <PairedDraft lang={lang} pair={TSHIVENDA_SAMPLE_CHOOSER_DRAFT.intro} /> : ui('Explore each workspace. Use Tour in the menu to return here.', 'Hlola indawo ngayinye yokusebenza. Sebenzisa uHambo kumenyu ukuze ubuyele lapha.')}</p>{lang === 'st' && <p role="note" style={{ display: 'block', marginBottom: 0, fontWeight: 700 }}>Sesotho machine draft · awaiting fluent review</p>}{lang === 've' && <p role="note" style={{ display: 'block', marginBottom: 0, fontWeight: 700 }}>Tshivenda machine draft · awaiting fluent review</p>}</div>
    {error && <p role="alert">{error}</p>}
    <section aria-label={lang === 'st' ? `${draftText(SESOTHO_SAMPLE_CHOOSER_DRAFT.sectionLabel)} (${SESOTHO_SAMPLE_CHOOSER_DRAFT.sectionLabel.sourceEnglish})` : lang === 've' ? `${draftText(TSHIVENDA_SAMPLE_CHOOSER_DRAFT.sectionLabel)} (${TSHIVENDA_SAMPLE_CHOOSER_DRAFT.sectionLabel.sourceEnglish})` : ui('All views', 'Zonke izindawo')} style={{marginBottom:24}}><h2>{lang === 'st' ? <PairedDraft lang={lang} pair={SESOTHO_SAMPLE_CHOOSER_DRAFT.sectionHeading} /> : lang === 've' ? <PairedDraft lang={lang} pair={TSHIVENDA_SAMPLE_CHOOSER_DRAFT.sectionHeading} /> : ui('Choose your workspace', 'Khetha indawo yakho yokusebenza')}</h2><p>{!choicesReady ? ui('Checking available views…', 'Kubhekwa izindawo ezitholakalayo…') : user && !accountRole ? ui('Your account role could not be confirmed. Refresh your access in Account, then return here.', 'Indima ye-akhawunti yakho ayiqinisekisiwe. Vuselela ukufinyelela kwakho ku-I-akhawunti, bese ubuya lapha.') : accountRole && !['ngo','admin'].includes(accountRole) ? ui(`Your ${accountRole} account opens its matching practice view. Other roles are shown below so the full set is visible.`, `I-akhawunti yakho ye-${accountRole} ivula indawo ehambisana nayo yokuzilolonga. Ezinye izindima ziboniswe ngezansi ukuze ubone zonke.`) : ui('Choose any of the five workspaces.', 'Khetha noma iyiphi yezindawo ezinhlanu zokusebenza.')}</p>
    <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}} aria-label={lang === 'st' ? `${draftText(SESOTHO_SAMPLE_CHOOSER_DRAFT.indexLabel)} (${SESOTHO_SAMPLE_CHOOSER_DRAFT.indexLabel.sourceEnglish})` : lang === 've' ? `${draftText(TSHIVENDA_SAMPLE_CHOOSER_DRAFT.indexLabel)} (${TSHIVENDA_SAMPLE_CHOOSER_DRAFT.indexLabel.sourceEnglish})` : ui('View index', 'Uhlu lwezindawo')}>{examples.map(([id,label])=><a key={id} href={`#sample-${id}`} style={{padding:'10px 12px',border:'1px solid var(--border)',borderRadius:20}}>{lang === 'st' ? <PairedDraft lang={lang} pair={SESOTHO_SAMPLE_CHOOSER_DRAFT.roles[id].label} inline /> : lang === 've' ? <PairedDraft lang={lang} pair={TSHIVENDA_SAMPLE_CHOOSER_DRAFT.roles[id].label} inline /> : label}</a>)}</div>
    <div className={styles.sampleGrid}>{examples.map(([sampleRole, label, title, description]) => { const Icon = icons[sampleRole]; const allowed = availableRoles.includes(sampleRole); const englishLabel = examplesEn.find(([id]) => id === sampleRole)?.[1] ?? label; const roleCopy = lang === 'st' ? SESOTHO_SAMPLE_CHOOSER_DRAFT.roles[sampleRole] : TSHIVENDA_SAMPLE_CHOOSER_DRAFT.roles[sampleRole]; return <article id={`sample-${sampleRole}`} key={sampleRole} className={`${styles.card} ${styles.sampleCard}`}><Icon size={24} aria-hidden="true"/><span className={styles.tag}>{lang === 'st' ? <PairedDraft lang={lang} pair={roleCopy.label} inline /> : lang === 've' ? <PairedDraft lang={lang} pair={roleCopy.label} inline /> : label}</span><h3 style={{margin:'8px 0'}}>{lang === 'st' ? <PairedDraft lang={lang} pair={roleCopy.title} /> : lang === 've' ? <PairedDraft lang={lang} pair={roleCopy.title} /> : title}</h3><p>{lang === 'st' ? <PairedDraft lang={lang} pair={roleCopy.subtitle} /> : lang === 've' ? <PairedDraft lang={lang} pair={roleCopy.subtitle} /> : description}</p><button type="button" disabled={!allowed} className={styles.primary} onClick={() => { if (!allowed) return; if (startRolePreview(sampleRole)) router.push('/' + sampleRole); else setError(ui('Tour mode could not start. Please allow session storage and try again.', 'Uhambo alukwazanga ukuqala. Vumela ukugcinwa kolwazi lweseshini bese uzama futhi.')); }}>{allowed ? ui(`Open ${lang === 'st' || lang === 've' ? englishLabel : label}`, `Vula: ${label}`) : !choicesReady ? ui('Checking access…', 'Kubhekwa ukufinyelela…') : ui('Not available for this account', 'Akutholakali kule akhawunti')}</button></article>; })}</div></section>
