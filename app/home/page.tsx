...
function FarmPlanCard({ places, mainSite }: { places: SavedPlace[] | null; mainSite: SavedPlace | null }) {
  ...
  return (
    <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 20, padding: '16px 18px' }}>
      ...
      <style jsx global>{`
        @keyframes imfPlanReveal { from { transform: scaleX(0); } to { transform: scaleX(1); } }
        .imf-plan-progress { transform-origin: left center; animation: imfPlanReveal 500ms ease-out both; }
        @media (prefers-reduced-motion: reduce) {
          .imf-plan-progress { animation: none; transition: none !important; }
        }
      `}</style>
    </div>
  );
