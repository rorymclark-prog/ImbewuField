...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100dvh', background: '#F7F2E9', padding: 24, textAlign: 'center' }}>
      ...
      <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 22, fontWeight: 600, color: '#20190F', marginBottom: 8 }}>Something went wrong</h2>
      <p style={{ fontFamily: 'system-ui, sans-serif', fontSize: 14, color: '#5C5040', marginBottom: 24 }}>
        {error.message || 'An unexpected error occurred.'}
      </p>
      <button
        onClick={reset}
        style={{ height: 44, padding: '0 24px', background: '#1F4D2B', color: '#F7F2E9', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
      >
        Try again
      </button>
    </div>
