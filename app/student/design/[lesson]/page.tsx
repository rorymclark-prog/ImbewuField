...
export default async function DesignLessonPage({ params }: { params: Promise<{ lesson: string }> }) {
  ...
  return <div className={styles.page}>
    <header className={styles.header}><MenuButton /><BackButton fallback="/student/design" /><OfflinePageLink href="/student/design">Design a homestead</OfflinePageLink><span>{lesson.code} · {index + 1} / {DESIGN_LESSONS.length}</span></header>
    <main className={styles.main}>
      ...
      <section className={styles.section}><p className={styles.eyebrow}>The busy yard · invented classroom example</p><h2>Follow the decision</h2><FinanceText text={section('Worked example')} /><div className={styles.actions}><OfflinePageLink href="/student/design/case">See the full case and source notes →</OfflinePageLink></div></section>
      <section className={styles.section} id="practice"><h2>Make the next part of your folder</h2><FinanceText text={section('Learner task')} /><h3>What your partner or facilitator should look for</h3><FinanceText text={section('Evidence for feedback')} /><div className={styles.actions}><OfflinePageLink href={`/student/design/folder#folder-${unit.id}`}>Record this stage in your design folder →</OfflinePageLink></div></section>
      {workedStage && <section className={styles.section}><p className={styles.eyebrow}>Guided fictional model</p><h2>See this stage in a complete decision chain</h2><p>The worked demonstration uses supplied classroom geometry and a named source pack. It is separate from the unmeasured busy-yard exercise.</p><div className={styles.actions}><OfflinePageLink href={`/student/design/worked#${workedStage[0]}`}>{workedStage[1]} →</OfflinePageLink></div></section>}
      {['d1-3', 'd4-1', 'd4-3'].includes(id) && <section className={styles.section}><p className={styles.eyebrow}>Visual practice · supplied model dimensions</p><h2>Read the scale and check the space</h2><p>Compare two bed outlines with a gap, touching edges and an overlap. Follow the supplied dimensions and check what the area total counts. This is a separate sample-model exercise, not a measurement of the busy-yard household.</p><div className={styles.actions}><OfflinePageLink className={styles.primary} href="/student/design/scale">Open the scale exercise →</OfflinePageLink></div></section>}
