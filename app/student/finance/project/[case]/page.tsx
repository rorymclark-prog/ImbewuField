...
export default async function FinanceProjectPage({ params }: { params: Promise<{ case: string }> }) {
  ...
  return <div className={styles.page}>
    <header className={styles.header}><MenuButton /><BackButton fallback="/student/finance" /><Link href="/student/finance">Farm Finance</Link><span>Practical project</span></header>
    <main className={styles.main}>
      ...
      <nav aria-label="Project cases" className={styles.actions}>{project.cases.map(entry => <Link key={entry.id} href={`/student/finance/project/${entry.id}`} aria-current={entry.id === id ? 'page' : undefined}>{entry.title}</Link>)}</nav>
      ...
      <section className={styles.section}><h2>Practise supported app tasks</h2><p>Use the sample farm, not your real financial records. Partial payments stay in this workbook exercise where the app cannot represent them fully.</p><div className={styles.actions}><Link href="/student/guides/mapping">Mapping guide</Link><Link href="/student/guides/harvest">Harvest guide</Link><Link href="/student/guides/invoices">Invoice guide</Link><Link href="/student/guides/payments">Payment guide</Link><Link href="/student/guides/expenses">Expense guide</Link></div></section>
      <section className={styles.section}><h2>Why these distinctions matter</h2><p>The cases and figures are original classroom material. The record categories and the distinction between cash movement and income are informed by <a href="https://www.fao.org/4/w6864e/w6864e0f.htm">FAO record-keeping guidance</a> and <a href="https://www.extension.iastate.edu/agdm/wholefarm/html/c3-25.html">Iowa State University’s farm income statement guidance</a>. Local tax rules and foreign worked examples are not imported into this exercise.</p><div className={styles.actions}><Link href="/student/finance">Return to the course outline</Link></div></section>
    </main>
  </div>;
}
