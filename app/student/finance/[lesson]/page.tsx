...
export default async function FinanceLessonPage({ params }: { params: Promise<{ lesson: string }> }) {
  ...
  return <div className={styles.page}>
    <header className={styles.header}><MenuButton /><BackButton fallback="/student/finance" /><Link href="/student/finance">Farm Finance</Link><span>{lesson.code} · {index + 1} / 24</span></header>
    <main className={styles.main}>
      ...
      <section className={styles.section}><h2>Use the app alongside this lesson</h2><p>Read the relevant guide, then practise in the sample farm. Keep workbook figures out of your real farm records.</p><div className={styles.actions}>{FINANCE_APP_GUIDES[unit.id].map(guide => <Link key={guide.href} href={guide.href}>{guide.title} →</Link>)}</div></section>
      ...
      {lesson.id === 'f8-3' && <section className={styles.section}><h2>Bring the course together</h2><p>Try the connected practice case, then work through an independent case with fresh figures.</p><div className={styles.actions}><Link href="/student/finance/project/guided">Open the practical project →</Link></div></section>}
      <section className={styles.section}><h2>Keep your place</h2><FinanceReadingChecklist lessonId={lesson.id} lessonIds={FINANCE_LESSONS.map(e => e.lesson.id)} />
        <nav aria-label="Finance lesson navigation" className={styles.actions}>{previous && <Link href={`/student/finance/${previous.id}`}>← {previous.title}</Link>}{next ? <Link className={styles.primary} href={`/student/finance/${next.id}`}>Next: {next.title} →</Link> : <Link className={styles.primary} href="/student/finance">Return to the course outline</Link>}</nav>
      </section>
    </main>
  </div>;
}
