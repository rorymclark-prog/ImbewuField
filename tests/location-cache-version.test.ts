...
test('the version is at least v5 — the corrected lowveld climate answer must replace old caches', () => {
  ...
  assert.ok(version >= 5, `location cache is at v${version}; the monthly-mean biome fix needs v5 or later`);
});
