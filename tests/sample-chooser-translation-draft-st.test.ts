...
test('the chooser visibly pairs every selected draft field and keeps English/isiZulu routing intact', () => {
  ...
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{SESOTHO_SAMPLE_CHOOSER_DRAFT\.eyebrow\}/);
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{SESOTHO_SAMPLE_CHOOSER_DRAFT\.heading\}/);
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{SESOTHO_SAMPLE_CHOOSER_DRAFT\.intro\}/);
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{SESOTHO_SAMPLE_CHOOSER_DRAFT\.sectionHeading\}/);
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{roleCopy\.label\}/);
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{roleCopy\.title\}/);
  assert.match(page, /<PairedDraft lang=\{lang\} pair=\{roleCopy\.subtitle\}/);
  assert.match(page, /lang === 'zu' \? examplesZu/);
  assert.match(page, /lang === 'st' \|\| lang === 've'/);
