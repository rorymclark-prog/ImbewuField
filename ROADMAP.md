...
- **Deterministic cartographic finishing engine (next quality session):** clean the final drawing without asking AI to infer topology. Snap only tiny, nearly aligned endpoint gaps between matching route types; snap adjacent zone edges and remove hairline seams without merging different zones; clip visual zones to the property boundary; render a measured break and posts where a placed Gate intersects the boundary fence; and gently round only soft landscape polygons. Saved geometry stays unchanged, and legal boundary, house and driveway footprints are never reshaped. Cover false joins, distinct route types, gate-away-from-boundary, overlapping zones and maximum movement tolerances with tests.
- **Design-aware Ask Lima (later session):** give Lima the current site location, climate/slope/sector context, boundary, house, driveway, zones, traced ground, every placed element and route with dimensions, and the active layer. Structured saved geometry is authoritative; a page image is supporting context only. Lima should answer questions such as whether rabbits suit this design and where they could go by considering shade, drainage, access, water, feed, neighbours, predators, compost links and everything already placed, then offer a clearly marked proposal for confirmation rather than silently adding or inventing anything.
- **Deep Design Studio element + map-flow audit (later session):** review every catalogue element's editor step, layer toggle, exact-sheet membership, AI-sheet vocabulary, label and legend; check the full Base → Sector → Water → Zones → Planting → Structures → Review → Glossy flow; make placement locations obvious (for example Gate under Structures); and add an element × step × map test matrix so ownership cannot drift. In the DESIGN EDITOR itself, greywater routes must be purple and clean-water pipe routes blue, not only in final renders. Include a pre-render Water completeness check that asks about source, storage, treatment, main route, delivery and destination without inventing any missing part.
