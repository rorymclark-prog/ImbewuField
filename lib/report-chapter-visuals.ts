 ...
 * `after` names the drawn figure the picture explains: in a chapter that holds that figure the
 * picture follows it directly, so a chapter covering two subjects reads subject by subject. */
const NOT_THIS_SITE='Concept illustration, the same in every report. It is not a picture of this site.';
const REPORT_ART:Array<{id:string;when:RegExp;body?:RegExp;after?:string;title:string;alt:string;note:string}>=[
  {id:'roof-to-tank',after:'water-budget',when:/water harvesting|water plan|water strategy|water, soil and|amanzi, umhlabathi/i,title:'Follow the water through the site',alt:'Rain runs off a roof into a gutter, through a first-flush pipe into a tank, and the overflow is led to a mulched basin around a fruit tree.',note:`${NOT_THIS_SITE} It does not confirm a tank is installed, a pipe is correctly sized or water is safe to use.`},
  {id:'swale-section',after:'water-budget',when:/water harvesting|water plan|water strategy|earthwork|slope/i,body:/swale|contour|berm/i,title:'How a swale holds water on a slope',alt:'Cut through a slope: runoff drops into a level ditch, soaks in and spreads under a fruit tree planted on the mound below it.',note:`${NOT_THIS_SITE} A swale must be set out level on the contour and sized for the slope; on steep or unstable ground get advice before digging.`},
  {id:'drip-bed',after:'water-budget',when:/irrigation/i,title:'Water at the roots, not on the leaves',alt:'A raised tank feeds drip lines laid under mulch along rows of cabbage and spinach.',note:`${NOT_THIS_SITE} Pipe sizes, tank height and watering times still have to be worked out for the real beds.`},
  {id:'soil-layers',after:'soil',when:/soil strategy|soil plan|soil management|water, soil and|amanzi, umhlabathi/i,title:'Look below the surface',alt:'Cut through garden soil: mulch on top, dark living topsoil with roots and earthworms, paler subsoil with stones below, and compost being added by hand.',note:`${NOT_THIS_SITE} Use the site evidence checklist to add soil observations and laboratory results. This illustration does not describe measured conditions at this site.`},
  {id:'trench-bed',after:'soil',when:/soil|planting|food production|production spaces|year 1/i,body:/trench/i,title:'Inside a trench bed',alt:'Cut through a trench bed: branches and bones at the bottom, then dry grass, green material, manure and topsoil, with cabbages growing in mulch on top.',note:`${NOT_THIS_SITE} Depth and fill depend on what the household has to hand.`},
  {id:'compost-bays',after:'soil',when:/soil|year 1/i,body:/compost/i,title:'Compost in three stages',alt:'Three compost bays side by side: fresh material, a heap half broken down, and finished dark compost ready for the beds.',note:`${NOT_THIS_SITE} Any container or heap works; the point is the three stages.`},
  ...
  {id:'windbreak-section',after:'sectors',when:/wind/i,title:'How a windbreak lifts the wind',alt:'Wind meets a belt of tall trees, smaller trees and shrubs, rises over it and comes down well beyond the vegetable beds sheltered behind.',note:`${NOT_THIS_SITE} Which side to plant follows the wind directions recorded for the site, not this drawing.`},
  {id:'sun-house',after:'sectors',when:/sun (&|and) solar|solar/i,title:'Low winter sun in, high summer sun out',alt:'A house with a wide eave: the high summer sun is stopped by the roof overhang while the low winter sun reaches in under it.',note:`${NOT_THIS_SITE} The real sun angles for this latitude are in the sun and wind figure.`},
...
export function chapterGraphics(heading:string, body:string, visuals:ReportVisuals, figures:ReportChart[]=[]):ChapterGraphic[] {
  ...
  for(const art of REPORT_ART.filter(a=>REPORT_ART_READY.has(a.id)&&a.when.test(title)&&(!a.body||a.body.test(body))).slice(0,2)){
    const graphic:ChapterGraphic={id:`art-${art.id}`,title:art.title,note:art.note,art:{src:reportArtSrc(art.id),alt:art.alt,width:REPORT_ART_SIZE[art.id]?.[0]??1600,height:REPORT_ART_SIZE[art.id]?.[1]??800}};
    let at=art.after?result.findIndex(g=>g.id===`figure-${art.after}`):-1;
    if(at<0){result.push(graphic);continue;}
    while(result[at+1]?.id.startsWith('art-'))at+=1;
    result.splice(at+1,0,graphic);
  }
