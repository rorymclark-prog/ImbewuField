...
const OVERLAY_ICONS: Record<string, string> = {
  ...
  tree:      'a tree marker → a canopy seen from above with a soft shadow to the lower-right, drawn to its species where the label names one: moringa feathery and pale, avocado dark glossy and dense, macadamia dense mid-green, citrus small round bright green flecked with orange fruit, orange small round dense green flecked with round orange fruit, lemon small round paler yellow-green flecked with oval yellow fruit, grapefruit a larger round dense dark-green crown flecked with large pale yellow-blushed fruit, mango broad and dark, pawpaw a crown of big lobed leaves on a bare stem, indigenous shade tree a broad flat-topped or dome-shaped crown of fine grey-green foliage spreading wider than it is tall — never conical, never a conifer, never a pine',
...
1b. GROUND NOBODY TRACED IS STILL REAL GROUND. The farmer traces what they had time to trace, and everything else in the photograph is just as real: vehicle tracks, yards, hardstanding, slabs, roofs, bare compacted earth. NEVER plant, pave, build or texture over any of them — no tree, canopy, bed, shrub, hedge or new surface may sit on a track, a yard, a roof or a slab that is visible in the photograph, whether or not anyone drew an outline around it, and no new slab, apron or paved area may be invented anywhere. A design element belongs where the design puts it and nowhere else; if the only free ground near a marker is a track, draw the marker smaller rather than moving it onto the track or planting across it.
...
export function buildFinishedSheetPolishPrompt(
  ...
  stylePreset: StylePreset,
  placeName?: string,
  // Computed sentence list from lib/structure-register.ts naming each traced feature and whether
  // it is a roofed building or a flat ground surface. Without it the model guessed, and both the
  // concrete slab and the driveway came back rendered as extra buildings.
  structureRegister?: string,
): string {
  const body = [
