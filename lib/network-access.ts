 ...
 * express "this funder funds this NGO" (funded_org_ids), so a client read is no
 * longer wide open. But rules cannot express "return the income figure only if
...
export function decideNetworkAccess(profile: CallerProfile | null): NetworkAccess {
  ...
  // the rule identical to firestore.rules `orgVisible`.
  const visible = new Set<string>();
  if (profile.org_id) visible.add(profile.org_id);
  if (profile.role === 'funder') {
    for (const id of profile.funded_org_ids ?? []) if (id) visible.add(id);
  }
